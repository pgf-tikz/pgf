...
% 	3. then,
% 	\pgfmanualprettyprinterhandlecollectedargs{<result>}{<found terminator>? 1 :0} will be
...
% 		\pgfmanualprettyprinterhandlecollectedargs{{_1 first argument }_2 {_1 second argument }_2 }{1}
%		\donnextstep
% 		next tokens
% 	
%
%
% ATTENTION:
% 	\pgfmanualprettyprinterhandlecollectedargs will be invoked with TWO
% 	arguments. The first argument as such will contain extra braces, one pair
...
% 		\pgfmanualprettyprinterhandlecollectedargs{{<arg>}}{1}
%	for one argument and with
% 		\pgfmanualprettyprinterhandlecollectedargs{{<first arg>}{<second arg>}}{1}
...
%   The second argument is currently unused; it is '1' if the final terminator was found and '0' otherwise.
%   The purpose of this second argument is to ensure that all such callbacks have the same semantics, even if argument counting might not need this information.
%
...
\def\pgfmanual@pretty@collectargs@loop#1{%
	...
	\ifx\pgfmanual@loc@TMPc\pgfmanual@EOI
		...
		\edef\pgfmanual@pretty@collectargs@loop@NEXT{%
			...
			\noexpand\pgfmanualprettyprinterhandlecollectedargs{\the\t@pgfmanual@allargs\the\t@pgfmanual@currentarg}{1}%
						...
						\ifnum\c@pgfmanual@pretty@openbracecount=0
							...
							\ifnum\c@pgfmanual@pretty@curargcount=\pgfmanual@loc@csargcount\relax
								\edef\pgfmanual@pretty@collectargs@loop@NEXT{%
									\noexpand\endgroup
									\noexpand\def\noexpand\pgfmanualprettyprinternumcollectedargs{\the\c@pgfmanual@pretty@curargcount}%
									\noexpand\pgfmanualprettyprinterhandlecollectedargs{\the\t@pgfmanual@allargs}{1}%
									\the\t@afterpgfmanualprettyprinterhandlecollected
								}%
							\fi
						\else
	...
	\ifx\pgfmanual@loc@TMPc\pgfmanual@EOI
		\edef\pgfmanual@pretty@collectupto@loop@NEXT{%
			...
			\ifnum\c@pgfmanual@pretty@openbracecount>0
				\noexpand\pgfmanualprettyprinterarghasunmatchedbracestrue
			\fi
			\noexpand\pgfmanualprettyprinterhandlecollectedargs{\the\t@pgfmanual@currentarg}{0}%
			\the\t@afterpgfmanualprettyprinterhandlecollected
			\noexpand\pgfmanual@EOI% <- put this token back into token list!
		}%
	\else
					...
					\else
						\ifx\pgfmanual@loc@TMPc\pgfmanual@loc@delimittoken
							\ifnum\c@pgfmanual@pretty@openbracecount=0
								...
								\edef\pgfmanual@pretty@collectupto@loop@NEXT{%
									\noexpand\endgroup
									\noexpand\pgfmanualprettyprinterhandlecollectedargs{\the\t@pgfmanual@currentarg}{1}%
									\the\t@afterpgfmanualprettyprinterhandlecollected
								}%
							\else
								\t@pgfmanual@currentarg=\expandafter{\the\t@pgfmanual@currentarg#1}%
...
\def\pgfmanual@pretty@collectargs@finish@cs#1#2{%
				...
				\else
					...
					\else
						\ifx\pgfmanual@loc@TMPb\pgfmanual@pretty@lbrace
							...
							% name)
							\def\pgfmanualprettyprinterhandlecollectedargs##1##2{%
								\toks1={##1}%
...
\def\pgfmanual@pretty@pgfkeys@collectvalue#1{%
	...
	\else
		...
		\if\pgfmanual@pretty@isconsumed0%
			\def\pgfmanualprettyprinterhandlecollectedargs##1##2{\pgfmanual@pretty@pgfkeys@processvalue{##1}}% the braces will be handled separately.
