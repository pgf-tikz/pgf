...
\begin{tikzpicture}[baseline]
  \datavisualization [
    ...
    clean ticks,
    all axes={
      grid={linear ats,minor steps between steps}
    },
    x axis={
      ticks={some,
        major={also at=
          {0.5*pi as $\pi/2$,
            {pi as
              [style={red},node style={text=blue},
              tick text at max extend,
              tick text at min extend=false] $\pi$}}}
      },
      grid={common={max extend=\pgfdvmax+0.1,min extend=\pgfdvmin-0.1}},
      label=$x$
    },
    y axis={
      label=$y$,
...
A very simple plot of a parabola:
...
\begin{tikzpicture}
  ...
  [
    school book plot,
    3d axes,
    x axis={attribute=sin},
    y axis={attribute=cos},
    z axis={attribute=t,axis length=1cm}
  ]
    data [format=function]
    {
      var t : interval[0:360];
      
      func sin = sin(\value t);
      func cos = cos(\value t);
    };
\end{tikzpicture}
...
\begin{tikzpicture}
  ...
  \datavisualization[candle stick plot,
                     index/source=dax]
    scope [column 1=dax/low,
           column 2=dax/high,
           column 3=dax/entry,
           column 4=dax/exit]
    {
      data [format=space separated columns]
      {% today
        2000 2300 2100 2200 
        2000 2350 2200 2500
        2200 2300 2250 2260 
