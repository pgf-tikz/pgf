...
\begin{key}{/tikz/data visualization/about=\meta{number}}
    ...
    However, $s$ will typically something strange like $0.02345$ or
    ...
    instance, $0.02345$ would be rewritten as $2.345 \cdot 10^{-2}$ and
    $345\,223.76$ as $3.4522376 \cdot 10^5$. The next step is to replace the
    still not-so-good number $m$ like $2.345$ or $3.452237$ by a ``good'' value
    $m'$. For this, the current value of the |about strategy| is used:
    %
...
\end{codeexample}
    %
    As can be seen, we can pass another axis as an \meta{option} to |visualize
    axis|, where we pass the following key to the axis in turn:
    %
...
\begin{key}{/tikz/data visualization/new axis system=\marg{axis system
    ...
    \begin{key}{/tikz/data visualization/\meta{axis system name}=\opt{\meta{options}}}
        ...
        %
        \begin{enumerate}
            \item The \meta{axis setup} with the path prefix |/tikz/data
                visualization/|.
