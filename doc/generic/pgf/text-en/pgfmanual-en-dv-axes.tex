...
\begin{key}{/tikz/data visualization/about=\meta{number}}
  ...
  maximum and the minimum value on the axis is computed; let us call
  it $r$ for ``range.'' Then, $r$ is divided by \meta{number},
  ...
  Z$. For instance, $0.02345$ would be rewritten as $2.345 \cdots
  10^{-2}$ and $345223.76$ as $3.4522376 \cdots 10^5$. The next step
  ...
  ten. In addition to the standard |about strategy|, there are some
  ...
  % Show the effect for the different strategies
    \medskip
    \begin{tikzpicture}
      \foreach \max in {10,20,...,100}
      {
        \begin{scope}[xshift=\max pt*3]
          \datavisualization [#1,
          ...
          \node at (0,-5mm) [anchor=mid] {\texttt{5}};
        \end{scope}
      }
  ...
  \begin{tikzpicture}
    \foreach \about in {1,...,10}
    {
      \begin{scope}[xshift=\about pt*30]
        \datavisualization [#1,
        one dimensional axis,
        axis={
          ticks={about=\about},
...
\begin{key}{/tikz/data visualization/none}
  Switches off the automatic step computation.
\end{key}
