...

{
  ...
  \node (left text) [text width=.5\paperwidth-2cm,below right,at={(-.5\paperwidth+1cm,-2cm)}]
  {
    \fontfamily{pcr}
...
\end{lstlisting}
  ...
  \node (right text) [text width=.5\paperwidth-2cm,below right,at={(1cm,-2cm)}]
  {
    \fontfamily{pcr}
    \def\textbraceleft{\char`\{}
    \def\textbraceright{\char`\}}
  ...
  \foreach \i in {0.5,0.6,...,2}
   ...
   \fill [decorate,decoration=gnome footprints]
