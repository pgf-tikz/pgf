...
\item
  ...
  speed: While linear coordinate trnasformations can be applied very
...
nonlinear transformations to, say, text, because the backend drivers
do not support this.)

This means that there is a fundamental
difference between calling a function like |\pgfpointpolar| or
specifying a coordinate as |(45:2)| in \tikzname\ and installing the
nonlinear transformation ``polar coordinates'' using the command
|\pgftransformnonlinear|: In a coordinate like |(45:2)| the user
explicitly says ``please evaluate this one 
coordinate in polar coordinate and then continue in the normal
coordinate system with the result.'' Otherwise nothing changes and a
line between two points specified in this way is still a straight
line.
...
Due to the way nonlinear transformations are implemented, not every
possible transformation is permissible. Rather, they should have the
following properties:
\begin{itemize}
\item The transformation should be continuous and differentiable. When
  drawing lines and curves, \pgfname\ will try to compute tangent
  vectors at many points. At points where this is not possible (such
  as at the origin for a polar transformation), wrong drawings will
  result.
\item Related to the problem of computing tangents, the nonlinear
  transformation must honour the \emph{handedness} of the coordinate
  system. This means, roughly, the ``right must stay right'': Imagine
  a line somewhere and a point right of the line. The nonlinear
  transformation will now transform the line to some new curve and
  will place the coordinate somewhere else. It must now be the case
  that the point is still to the right of the curve, it may not
  ``switch sides.'' An example of a transformation that does not
  honour handedness, we again have a look at polar transformations:
  Consider a line from $(0,1)$ to $(90,1)$ and a point at
  $(45,0.5)$. Clearly, in normal coordinate space, this point is ``to
  the right'' of the line as we travel from $(0,1)$ to $(90,1)$. Now,
  let us transform this line via a polar transformation: The first
  component is the angle and the second the distance, so the line
  becomes an arc at radius $1$ from $0^\circ$ to $90^\circ$. The point
  is transformed to $(1/\sqrt 2, 1/\sqrt 2)$ and it now lies \emph{to
    the left} of the curve as we travel along it. On the other hand,
  if we consider a line with negative $y$-coordinates such as from
  $(0,-1)$ to $(90,-1)$, the handedness is correct.
\end{itemize}
...
\begin{command}{\pgftransformnonlinear\marg{transformation code}}
  ...
  The job of the \meta{transformation code} is to map a point given in
  the registers |\pgf@x| and |\pgf@y| to a new coordinate, which
  should be returned in |\pgf@x| and |\pgf@y| as well. The mapping
  must honour the conditions listed above (continuous, differentiable,
  honouring handedness). As an example, suppose we wish to install
  polar coordinates (but with the angle going clockwise to ensure the
  handedness condition). For this, we need a bit of code:
\begin{codeexample}[code only]
\pgftransformnonlinear{%
  \pgfmathsincos@{-\pgf@x}% negate for handedness
  \pgf@x=\pgfmathresultx\pgf@x%
  \pgf@y=\pgfmathresultx\pgf@y%
}
\end{codeexample}
  ...
  becomes the typical ``polar grid'' (note that we do not try to draw
  something near the origin since, there, the mapping is not
  differentiable):
\makeatletter
\begin{codeexample}[]
\tikz{
  ...
  \draw (0,1mm) grid [xstep=10pt, ystep=5mm] (90pt, 20mm);
}
\end{codeexample}
  To be continued...
\end{command}
