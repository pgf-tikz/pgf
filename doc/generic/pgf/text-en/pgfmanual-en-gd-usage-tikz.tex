...
\begin{key}{/graph drawing/cluster=\opt{\meta{name}}}
  \keyalias{tikz}
  \keyalias{tikz/graphs}
  When this key is used inside a scope, all nodes and edges inside the
  scope will belong to a cluster named \meta{name}. If the key is used
  again in a different scope, but with the same \meta{name}, the nodes
  and edges of the other scope will also become part of the
  cluster. Consider the following example:
\begin{codeexample}[code only]
\graph {
  a, b[cluster=c2], c[cluster=c1], d[cluster=c2], e[cluster=c1],
  { [cluster=c2]
    f,g
  },
  b [cluster=c1]  
...
\end{codeexample}
  The following nodes will be part of cluster |c1|: |b|, |c|, and
  |e|. The nodes |b|, |d|, |f|, and |g| will be part of |c2|.

  When the \meta{name} is not given, a unique new name will be
  generated automatically. This makes it easy to create several
  clusters, as in the following example:  
  \begin{codeexample}[]
\tikz [rounded corners] \graph [layered layout] {
    1972 -> 1976 -> 1978 -> 1980 -> 1982 -> 1984 -> 1986 -> 1988 -> 1990 -> future;
        
    { [cluster]  1972, Thompson };
    { [cluster]  1976, Mashey, Bourne },
    { [cluster]  1978, Formshell, csh },
    { [cluster]  1980, esh, vsh },
    { [cluster]  1982, ksh, "System-V" },
    { [cluster]  1984, v9sh, tcsh },
    { [cluster]  1986, "ksh-i" },
    { [cluster]  1988, KornShell ,Perl, rc },
    { [cluster]  1990, tcl, Bash },
    { [cluster]  "future", POSIX, "ksh-POSIX" },
