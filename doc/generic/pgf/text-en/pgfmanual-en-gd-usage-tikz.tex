...
\begin{command}{\usegdlibrary\marg{list of libraries}}
  ...
  \item Check whether \TeX\ can find a file called
    |pgf.gd.|\meta{name}|.library.lua| using \TeX's usual file search
  ...
  \item Otherwise, check whether there is a file
    |pgf.gd.|\meta{name}|.lua|. If so, read it using |require|.
  \item Otherwise, check whether there is a file
    \meta{name}|.library.lua|. If so, read is using |require|.
  \item Otherwise, apply |require| to the file \meta{name}|.lua|. If
