...
\end{codeexample}
  ...
  are allowed:
  \begin{itemize}
  \item |start=|\meta{code}. The \meta{code} will be exected whenever
    |\pgfplotstreamstart| is issed while the handler \meta{macro} is
...
\end{codeexample}
  ...
  \item |point=|\meta{code}. The \meta{code} will be exected whenever
...
\begin{codeexample}[]
\pgfdeclareplothandler{\myhandler}{#1}{
  point=\pgfpathcircle{##1}{#1} % ##1 is the coordiante,
