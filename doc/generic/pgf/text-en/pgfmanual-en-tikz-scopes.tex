...
\begin{environment}{{tikzpicture}\opt{animations spec}\opt{\oarg{options}}}
...
\begin{command}{\tikz\opt{animations spec}\opt{\oarg{options}}\marg{path commands}}
...
\begin{environment}{{scope}\opt{animations spec}\opt{\oarg{options}}}
...
\begin{plainenvironment}{{scope}\opt{animations spec}\opt{\oarg{options}}}
  Plain \TeX\ version of the environment.
\end{plainenvironment}

\begin{contextenvironment}{{scope}\opt{animations spec}\opt{\oarg{options}}}
...
\begin{command}{\scoped\opt{animations spec}\opt{\oarg{options}}\meta{path command}}
...
the following:
\begin{codeexample}[code only]
\tikz[automaton] \graph { a[state, initial] -> b [state] -> c[state, final] };
\end{codeexample}
...
\begin{key}{/tikz/rdf=\meta{rdf keys}}
  By default, this key does nothing and just ``throws away'' the
  \meta{annotation keys}. The idea is that you can use this key (a
  ...
  in the \meta{rdf keys} cause annotations to be added to the
  output.
...
\end{codeexample}
  ...
  \begin{key}{/tikz/rdf/about=\marg{text}}
    Adds the \textsc{rdf} attribute |about="|\meta{text}|"| to the
  ...
  \begin{key}{/tikz/rdf/content=\marg{text}}
  \end{key}

  \begin{key}{/tikz/rdf/datatype=\marg{text}}
  \end{key}

  \begin{key}{/tikz/rdf/href=\marg{text}}
  \end{key}

  \begin{key}{/tikz/rdf/inlist}
  \end{key}

  \begin{key}{/tikz/rdf/prefix=\marg{text}}
  \end{key}

  \begin{key}{/tikz/rdf/property=\marg{text}}
  \end{key}

  \begin{key}{/tikz/rdf/rel=\marg{text}}
  \end{key}

  \begin{key}{/tikz/rdf/resource=\marg{text}}
  \end{key}

  \begin{key}{/tikz/rdf/rev=\marg{text}}
  \end{key}

  \begin{key}{/tikz/rdf/src=\marg{text}}
  \end{key}

  \begin{key}{/tikz/rdf/typeof=\marg{text}}
  \end{key}

  \begin{key}{/tikz/rdf/vocab=\marg{text}}
  \end{key}
\end{key}
