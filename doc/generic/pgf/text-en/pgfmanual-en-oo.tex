...
\begin{command}{\pgfoonew\meta{object handle}|=new |\meta{class
...
\end{codeexample}
  ...
  The \meta{object handle} is a normal \TeX\ macro name that will
...
\begin{command}{\method \meta{method name}|(|\meta{parameter list}|)|\marg{method body}}
  ...
  methods are called |id| and |object|, which are always automatically
...
\item Using the |object| method to create a handle to a given object. 
\end{enumerate}
...
\begin{predefinedmethod}{id(\meta{macro})}
  Calling \meta{obj}|.id(|\meta{macro}|)|  stores the id \meta{obj} in
...
\begin{predefinedmethod}{object(\marg{macro name})}
  ...
  \item |\obj.object(|\meta{macro name}|)|    
  \end{enumerate}

  The first method is simpler and faster. However, for |\pgfoothis|
  there is a difference: The call |\pgfoothis.object(|\meta{macro name}|)| will
...
\begin{ooclass}{signal}
  ...
  might connad the method |emergency action(#1)| to this signal, and
  so on.
