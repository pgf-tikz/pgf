...
\item
  ...
  First, if the \meta{node name} is just an underscore, then a fresh
...
\tikz \graph [grow down=5mm, branch right=5mm] { bintree [level=5] };
\end{codeexample}
\end{key}
...
\tikz \graph [grow up sep] {
  a [as={$a=x$}] --
  b [as={$b=\displaystyle \int_0^1 x dx$}] --
...
\end{quote}
As this specification is being parsed, the |graph| library will keep
...
\tikz \graph {
  a ->
  b[as=\pgfkeysvalueof{/tikz/graphs/placement/element count}] ->
  c[as=\pgfkeysvalueof{/tikz/graphs/placement/element count}]
};
\end{codeexample}
\end{key}
...
\begin{key}{/tikz/graphs/placement/width}
  This key stored the ``logical width'' of the nodes parsed up to now
...
\begin{graph}{subgraph I\char`\_n}
