...
There are two ways in which you can use the |graph| path command:
First, you can just use |graph| somewhere on a path at any place where
you could also use a command like, say, |plot| or |--|. Second, you
can use the command |\graph| which is just an abbreviation for
|\path graph|. 
...
\begin{pathoperation}{graph}{\opt{\oarg{options}}\meta{group specification}}
  ...
  \item Edges between the nodes may be added. They are added in the
    ...
    where the |graph| command is begin used.
  \end{enumerate}
...
that is either newly created to exists already, but they may also
...
nodes should be connect is in detail in 
...
\item The \meta{options} are executed (inside a local scope) with the
  ...
  specify how the edge should look like.
...
\item For each pair $(l_i,r_i)$ an edge is created. This is done by
  ...
  \begin{key}{/tikz/graphs/new ->=\marg{left
        node}\marg{right node}\marg{edge options}\marg{edge nodes}}
...
otherwise we say that the node is \emph{fresh}. This behaviour of
...
following way inside a protecting scope. For this, the current
...
\item 
  ...
  \begin{enumerate}
  \item The options that have accumulated in called to |nodes| from
    the surrounding scopes.
  \item The local \meta{options}.
  \end{enumerate}
  Note that, for once, all of the options are executed with the path
  prefix |/tikz/| rather than |/tikz/graphs/|, so you can directly use
  keys like |thick| or |font| inside the \meta{options}. (Actually,
  this is not quite true. In reality, all of the options are executed
  with the path prefix |/tikz/graphs|. However, a default
  handler for unknown keys with this prefix is installed that will
  retry the key with the prefix |/tikz/| when the key is not found
  inside |/tikz/graphs|. This means, in essence, that some more
  esoteric keys are more difficult to use inside the options and that
  any key with the prefix |/tikz/graphs| will take precedence
  over a key with the prefix |/tikz|).
\item The \meta{text} that is passed to the |\node| command is
  computed as following: First, you can use the following key to
...
which connecting edges are added. This is detailed in the next
section.
...
\subsection{Graph Expressions and Color Classes}
...
\subsubsection{Advanced: Graph Theoretic Background}

Let us now have a more detailed look at what is going on behind the
scenes. For this, let me start with some concepts from graph theory
that will help in describing how the |graph| library works.
...
a multi-set $E \subseteq V \times V$ of edges, and for each color a
subset $C \subseteq V$ of colors.
...
a certain graph~$G$. Given a graph expression $t$, we write $G(t)$ for
the graph it evaluates to.

First, $\iota(x)$ is an atomic expression, where $x$ is a name for a
node. Then $G(\iota(x))$ is the graph consisting of the single
node~$x$ with no colors.

Second, given two graph expressions $t$ and $t'$, then $t \cup t'$ is
also a graph expression. It evaluates to the graph $G(t \cup t') =
G(t) \cup G(t')$, where for $G = (V,E, C_1,\dots, C_m)$ and $G'= (V',
E', C_1',\dots,C_m')$ the multi-colored graph  $G \cup G'$ is given by
$(V \cup V', E \cup E', C_1 \cup C_1', \dots, C_m \cup C_m')$. 

Third, given a graph expression $t$ and a (logical) color~$i$, then
$c_i(t)$ and $\bar c_i(t)$ are also a graph expression. The graphs
$G(c_i(t))$ and $G(\bar c_i(t))$ are identical to $G(t)$, except that
all nodes have either additionally color~$i$ or have color~$i$
removed, respectively. (Recall that a node can have multiple colors.)

Fourth, given a graph expression $t$ and two logical colors $i$
and~$j$, then $\rho_{i \to j}(t)$ is a graph expression. The graph
$G(\rho_{i \to j}(t))$ is identical to $G(t)$, except that every node
that used to be colored by $i$ is now (instead) colored by~$j$.

Fifth, given a graph expression $t$ and two logical colors $i$
and~$j$, then $\eta_{i,j}$ is also a graph expression. The graph
$G(\eta_{i,j}(t))$ is identical to $G(t)$, except that there is an
(additional) edge from every node with color $i$ to every node with
color~$j$.
...
where $x$ is a name for a node. Every
Let $\iota(x)$ denote the graph that consists just of a single
node~$x$. Given two graphs $G$ and $H$, let $G \cup H$ be the graph
expression denote the
graph 

 From \tikzname's point of view, a graph is constructed
recursively as following: 
\begin{enumerate}
\item A single node is a graph.
\item Given a list of graphs, when you form a \emph{group} of them (by
  surrounding the list by curly braces and separating the graphs using
  commas or semicolons), what results is a graph that is simply the
  union of these graphs. 
\item Given two graphs, when you put an edge specification between
  them, then the two graphs are also united as in a group.
\end{enumerate}

In all of the above cases, each time a graph has been newly formed, a
set of \emph{operator} are applied to the resulting graph before it is
fed to the next recursion level. These operators can do different things:
\begin{itemize}
\item They can (re)assign colors to nodes, see below.
\item They can add edges between nodes of the just-constructed graph,
  see even further below.
\end{itemize}

From a formal point of view, a graph specification can be thought of
as a way of writing down graph expressions, which in turn evaluate to
graphs. 

\begin{enumerate}
\item A direct node specification like |x [as=bar, draw]| is mapped
  to a graph consisting of a single node named |x| that has a
  certain appearance (being drawn and showing the text ``bar''), but
  this appearance is irrelevant for the graph expression.

  The graph expression that yields this node is often denoted
  $\iota(x)$.
\item Consider a group node specification like |{ A, B, C }| and
  suppose that |A| is represented by the graph expression $\eta(A)$, 
\end{enumerate}


Let us now have a look at the colors.
...
overlapping \emph{color classes}. So, unlike when is sometimes called a
...
\begin{key}{/tikz/graphs/color class=\meta{color class name}}
  This sets up a new color class called \meta{color class name}. Nodes in a graph
  can now be made part of this class.

  For this, several new keys become available inside the current
  scope: 
  \begin{key}{/tikz/graphs/operators/\meta{color class name}}
    This key can be used with the |operator| command. It will cause the
    node to get the ``logical color'' \meta{color class name}. The
    node will retain this color in all encompassing scopes, unless it
    is explicitly changed (see below) or unset (again, see below).
  \end{key}
\end{key}
Once a color class has been declared inside a scope, new options
become available inside this scape.

Each group specification of a graph refers to certain subset
of the nodes of the graph (possibly all nodes of graph). The |graph|
command allows you to apply a \emph{connector} to the nodes of the
group, which will cause a number of edges to be added between the
nodes of the group.

Let us start with the easiest connector: The |clique| connector. This
connector simply adds edges between all nodes of the current group. 
