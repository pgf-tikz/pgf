...
In the following, color classes and graph operators are first
explained in an informal way. Interested readers will find a more
formal treatment of graph expressions at the end of this section.
...
\subsubsection{Simple Graph Operators}

Graph operators are applied to the nodes of a set of nodes. As
explained in 
Section~\ref{section-library-graphs-node-spec}, you can form such a
set by forming a group of nodes, which is nodes surrounded by curly
braces. Also, when you \emph{connect} two groups by putting on of the
five possible edge specification |--|, |<-|, |->|, |<->|, or |-!-|
between them, a larger set of nodes results. In all these cases, you
can apply a graph operator to the set.

\begin{key}{/tikz/graphs/operator}
  To be written...
\end{key}

\subsubsection{Joining Graph Operators}

\begin{key}{/tikz/graphs/default edge operator}
  To be written...
\end{key}
\begin{key}{/tikz/graphs/default edge kind}
  To be written...
\end{key}
...
The following needs to be rewritten / modified...
...
\item[Direct node specification] The syntactic construct
  ...
  graph expression
  $\mu_{\kappa_{\mathtt{all}^{\{\meta{text}}}}(\mu_{\kappa_{\mathtt{source}^{\{\meta{text}}}}(\mu_{\kappa_{\mathtt{target}^{\{\meta{text}}}}
...
\item[Group specifications] The syntactic construct
  ...
  is 
  $$\mu_{\meta{$op_n$}}\bigl(\dots\bigr(\mu_{\cup}(t_1,\dots,t_m)\bigr)\dots\bigr).$$
  Typically, the operators \meta{$op_i$} will edges between the nodes
  colored |all|. 
