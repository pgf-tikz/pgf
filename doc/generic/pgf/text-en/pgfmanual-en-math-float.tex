...
\begin{command}{\pgfmathfloattoregisterstok{\marg{x}}{\marg{flagscount}}{\marg{mantissetoks}}{\marg{exponentcount}}}
	A variant of |\pgfmathfloattoregisters| which writes the mantisse into a token register \marg{mantissetoks}. It maintains the full input precision.
\end{command}
...
\begin{command}{\pgfmathroundto{\marg{x}}}
	...
	As a side effect, the global boolean |\ifpgfmathfloatroundhasperiod| will be set to true if and only if the resulting mantisse has a period. Furthermore, the global boolean |\ifpgfmathfloatroundmayneedrenormalize| will be set to true if and only if the rounding result's floating point representation would have a larger exponent than \marg{x}. 
\begin{codeexample}[]
\pgfmathroundto{1}
\pgfmathresult
\end{codeexample}
