...
The scoping does not affect (as one would expect) the position of the
current point of the path. Consider the following example:

\begin{codeexample}[]
\tikz \draw (0,0) { -- ++ (1,1) } {-- ++ (1,0) };
\end{codeexample}
One might have expected that the |++ (1,0)| should haven computed
relative to the last point before the first scope, but it is relative
to the last point in this scope.
