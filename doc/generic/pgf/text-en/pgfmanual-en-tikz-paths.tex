...
\end{codeexample}
  \item
    There is a special option that allows an option to add something
    to the subsequent path:
    \begin{key}{/tikz/after option path=\meta{path}}
      When this key is used inside graphics options on a path, the
      \meta{path} is inserted right after the option block into the
      path. When you use this key multiple times inside graphic
      options, the effect accumulated.

      In the following example, we use a style to add little circles
      to a path.
\begin{codeexample}[]
\tikz [c/.style={after option path={circle (2pt)}}]
  \draw (0,0) -- (1,1) [c] -- (3,2) [c];
\end{codeexample}
      The effect is the same as of
      |(0,0) -- (1,1) circle (2pt) -- (3,2) cirlce (2pt)|.
    \end{key}
  \end{enumerate}
