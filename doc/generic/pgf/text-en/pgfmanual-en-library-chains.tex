...
\begin{command}{\chainin |(|\meta{existing name}|)| \opt{\oarg{options}}|;|}
  ...
  \meta{options}. However, then, you must write |on chain=|\meta{chain
    name} and you may not provide a \meta{direction} (which would not
  make sense anyway, the \meta{existing name} node has already been
  positioned). The \meta{existing node} will then be made part of
  \meta{chain name} rather than the default chain.
  
  Conceptually, this command is just a shortcut for
\begin{quote}
|\path (|\meta{existing name}|) [late options={on chain,|\meta{options}|}];|
\end{quote}
