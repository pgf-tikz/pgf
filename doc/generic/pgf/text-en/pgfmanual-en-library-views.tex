...
\begin{key}{/tikz/meet=\meta{to-be-viewed corner}
  ...
  missing, the \meta{window corner}s are assumed to be the same as the
  \meta{to-be-viewed corner}s. The latter are two corners of a
...
\end{codeexample}
  ...
  animations. In order to animate a view, you specify the scope
  containing the |meet| command as the target object and then animate its
  |:view| attribute:
\begin{codeexample}[]
\tikz [animate = {
  my scope:view = {
...
\end{codeexample}
  ...
  |animate me| key when you specify the animation inside the scope: 
\begin{codeexample}[]
\tikz [animate = {
  my scope:view = {
  }}] {
