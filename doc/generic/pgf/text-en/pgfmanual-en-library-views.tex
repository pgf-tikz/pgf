...
\end{codeexample}
    ...
    key when you specify the animation inside the \todosp{the following
    `codeexample' causes an error, so I commented it.} scope:
  %
% TODOsp: causing the error: "I don't know the key '/tikz/animate myself:' ..."
%         This was fixed to: `animate = { myself:view = {'
%         but now it gives the error message "! Illegal unit of measure (pt inserted)."
%         Interestingly only in the manual. TeXing the code in an MWE works fine.
%\begin{codeexample}[]
%\tikz [animate = {
%  my scope:view = {
%  }}] {
