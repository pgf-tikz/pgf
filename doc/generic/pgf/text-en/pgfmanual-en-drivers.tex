...
\begin{filedescription}{pgfsys-dvips.def}
    ...
    \begin{enumerate}
        \item In \LaTeX\ mode it uses |graphicx| for the graphics inclusion.
            ...
            processed with |ps2pdf| to produce PDF.
        \item In plain \TeX\ mode it does not support image inclusion.
        \item Axial and radial shadings are fully implemented. The same output
            quality (smooth shadings) is achieved in the PostScript output as
            with drivers that produce PDF output. For the lack of a direct
            counterpart in the PostScript language specification, functional
            shadings need to be approximated. They are of somewhat lesser quality
            than the functional shadings from PDF producing drivers.
