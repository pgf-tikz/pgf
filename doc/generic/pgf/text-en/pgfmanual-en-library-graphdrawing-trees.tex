...
Currently, however, the choice of algorithms available in this library
is somewhat limited, since noone has yet found the time and energy to
implement some advanced algorithms. Anyone wanting to write a master's
thesis, please contact me\dots


\subsection{Standard Tree Layout}

\begin{gdalgorithm}{tree layout}{modular layout}
  This is a standard all-purpose algorithm for computing layouts for
  trees. Currently, since no good special algorithm has been
  implemented, the standard layered layout algorithm is actually used,
  but this will change.
\begin{codeexample}[]
\tikz \graph [tree layout] { a -> {b,c -> {d,e,f}} };    
\end{codeexample}
\end{gdalgorithm}
