...
\begin{environment}{{colormixin}\marg{mix-in specification}}
    ...
    %
% TODOsp: this `codeexample` is not collected by the `extract.lua` script
...
\begin{command}{\colorcurrentmixin}
    ...
    %
\begin{codeexample}[
    pre={ % !!! `\usepackage{xxcolor}` needs to be added to the preamble
 % !!! (the resulting PDF contains spurious "-6pt-6pt" before the `minipage`)
    },
]
