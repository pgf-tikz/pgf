...
\end{pgfpicture}
\end{codeexample}
  \item \declare{|stroke|} or \declare{|draw|} both
    stroke the path and set the |tips| option. See
...
\end{codeexample}
  \item \declare{|tips|}
    adds arrow tips to the path. When only this option is given, but
    not |stroke|, the arrow tips are drawn, but the path is
    discarded. See Section~\ref{section-tips} for details.
  \item \declare{|clip|}
    clips all subsequent drawings against the path. Always supresses
    |tips|. See Section~\ref{section-clip} for further details. 
\begin{codeexample}[]
\begin{pgfpicture}
  \pgfpathmoveto{\pgfpointorigin}
  \pgfpathlineto{\pgfpoint{1cm}{1cm}}
...
ends, namely when the |tips| or the |stroke| key has been used with
with |\pgfusepath| (and not |clip|, which switches tips off). Even
when |tips| is used, \pgfname\ will add arrows only if the path does
not contain any closed subpaths and is not empty. Furthermore, arrow
tips will only be added to the last subpath, see
Section~\ref{section-arrow-tips-where} for some examples.
