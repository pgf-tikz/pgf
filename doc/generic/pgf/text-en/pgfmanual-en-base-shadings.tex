...
\end{codeexample}
    ...
    listed in the list changes (this includes effects like color mixins).
    \todosp{`mixins' --> `mixings'? (ff) (or is here really the object meant?)}
    Thus, when you specify a \meta{color list}, whenever the shading is used,
    ...
    into account. If the resulting \textsc{rgb} triples have not yet been
    used, a new shading is internally created and used. Note that if the option
...
\begin{command}{\pgfuseshading\marg{shading name}}
    ...
    in a |pgfpicture| environment, you should put a |\pgfbox| around it.
    %
\begin{codeexample}[]
\begin{pgfpicture}
