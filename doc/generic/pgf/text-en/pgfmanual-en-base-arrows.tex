...
In the above description, there are a number of occurrences of the word "appropriately".
...
arrow tip can vary according to the line width of the arrow tip being
...
\begin{command}{\pgfarrowsdeclare\marg{start name}\marg{end
  ...
  transformation is set up beforehand to a rotation such that when an
...
\end{codeexample}
  ...
  For these reasons, \pgfname\ will not simply execute your arrow
...
\end{codeexample}
  ...
  Here is a picture that visualizes the arrow extends. The
  ...
  that specifies the size. Here is the final code for the |arcs''| arrow
  tip:
\begin{codeexample}[]
\pgfarrowsdeclare{arcs''}{arcs''}
{
...
\end{codeexample}
  ...
  defined below has to set up the line width not for the ``main'' line
...
\end{codeexample}
  ...
  \medskip
  \textbf{Arrow options.}
  You may wish to have further options to influence the appearance of an
...
\begin{command}{\pgfarrowsdeclarereversed\marg{start name}\marg{end
  ...
  existing arrow kind. The (automatically created) code of the new
...
\begin{command}{\pgfsetarrowsstart\marg{start arrow kind}}
  ...
  strokes in the current \TeX-group. If \meta{start arrow kind}
