...
\begin{key}{/tikz/cs/first path name=\meta{path name}}
  This key specifies the first path name.
\end{key}

\begin{key}{/tikz/cs/second path name=\meta{path name}}
  This key specifies the second path name.
\end{key}

\begin{key}{/tikz/cs/path names=\meta{name 1}| and |\meta{name 2}}
  This key specifies the first and second path names. The spaces
  around |and| must always be included. Any other spaces will
  be assumed to be part of \meta{name 1} or \meta{name 2}.
\end{key}
...
\begin{tikzpicture}[thick]
  ...
  \fill [red, opacity=0.5]
    (intersection cs:path names=circle and curve)
     circle (2pt);
\end{tikzpicture}
\end{codeexample}
...
\begin{command}{\tikzintersectnamedpaths\opt{|[|\meta{options}|]|}\marg{path name 1}\marg{path name 2}}
  
  This command finds all the intersections of the paths associated 
  with \meta{path name 1} \meta{path name 2}. 
  By default, when no \meta{options} are specified, the number of 
  solutions will be stored in the \TeX-macro 
  |\solutions|.   
  When the options are executed the key path is set to |/tikz/cs/|
  and the \meta{option} keys are executed. However, the only key 
...
\begin{key}{/tikz/cs/id=\meta{name}}
  ...
  associated with \meta{name}. In addition, the number of solutions 
  will be stored in the \TeX-macro |\|\meta{name}|solutions|, so, 
  ...
  of solutions is required, \meta{name} should be a single word 
  consisting only of alphabetic characters. If not, then \meta{name}
...
\end{command}
  ...
  There is a key to invoke |\tikzintersectnamedpaths| inside a path. 
  However, the solutions will only be remembered until the end of the 
  path.
  
\begin{key}{/tikz/intersect named paths=\marg{keys}}
 
 This key sets the key path to |/tikz/cs|, executes \meta{keys} and
 then calculates the intersections on the basis of the parameters
 set by \meta{keys}. The |first path name|, |second path name|
...
\begin{tikzpicture}[every node/.style={opacity=1, circle, black, above left}]
  ...
  \fill [red, opacity=0.5]
    [intersect named paths={path names=ellipse and rectangle}]
