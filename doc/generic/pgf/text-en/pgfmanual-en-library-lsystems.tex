...
\draw [green!50!black, rotate=90]
  [l-system={rules={F -> FF-[-F+F]+[+F-F]}, axiom=F, order=4,
   step=2pt, randomize step=50, angle=30, randomize angle=5}]
  lindenmayer system;
\end{tikzpicture}
\end{codeexample}
...
\begin{key}{/pgf/lindenmayer system/randomize step=\meta{percentage} (initially 0)}
...
\begin{key}{/pgf/lindenmayer system/randomize angle=\meta{percentage} (initially 0)}
...
\begin{command}{\pgflindenmayersystem\marg{name}\marg{axiom}\marg{order}}
  Run the L-system called \meta{name} using the input string \meta{axiom}
...
\begin{tikzpicture}
  \draw [help lines] grid (3,2);
  \draw (0,0) l-system 
    [l-system={Hilbert curve, step=2pt, axiom=L, order=4, anchor=south west}];
  \draw (3,2) l-system 
    [l-system={Hilbert curve, step=2pt, axiom=L, order=4, anchor=north east}];
\end{tikzpicture}
\end{codeexample} 
\end{key}
