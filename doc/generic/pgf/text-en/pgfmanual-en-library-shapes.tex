...
\begin{pgflibrary}{shapes}
  This library packages just includes the sublibraries
  |shapes.geometric|, |shapes.misc|, |shapes.symbols| and is provided
  for historical reasons only. Normally, you should include the more
  specialized libraries described in the rest of this section.
\end{pgflibrary}
...
\subsection{Shadowed Shapes}

\begin{pgflibrary}{shapes.shadowed}
  This library defines shapes that come with a (partly) transparent
  shadow behind them. 
\end{pgflibrary}
...
\begin{shape}{circle with shadow}
  This shape looks exactly like a |circle| (and it has exactly the
  same anchors), but a ``shadow'' is added behind it. This shadow is
  be rendered by drawing a slightly larger and offset circle with a
  special fading in force, causing the shadow color to smoothly blend
  into the background. 

\begin{codeexample}[]
\begin{tikzpicture}
  \foreach \i in {1,...,8}
    \node[circle with shadow,draw=blue,fill=blue!20,thick]
      at (\i*45:1) {Circle \i};
\end{tikzpicture}
\end{codeexample}

  The shadow is influenced by the following keys:

  \begin{key}{/pgf/shadow scale=\meta{factor} (initially 1.05)}
    The shadow is \meta{factor} larger than the circle itself. A
    slightly larger shadow looks better.
  \end{key}

  \begin{key}{/pgf/shadow xoffset=\meta{dimension} (initially 2pt)}
    The shadow is horizontally offset by \meta{dimension} with respect
    to the circle itself.
  \end{key}

  \begin{key}{/pgf/shadow yoffset=\meta{dimension} (initially 2pt)}
    The shadow is vertically offset by \meta{dimension} with respect
    to the circle itself.
  \end{key}

  \begin{key}{/pgf/shadow color=\meta{color} (initially black!50)}
    The shadow color. Note that since the shadow fades out, the actual
    shadow will be lighter than this color.
  \end{key}

  By varying the parameters, interesting effects can be achieved. For
  instance, setting the offsets to |0pt| and slightly enlarging the
  scale yields a glow effect.
\begin{codeexample}[]
\begin{tikzpicture}
  \foreach \i in {1,...,8}
  \node[circle with shadow,fill=black,text=green!50!black,shadow scale=1.15,
        shadow xoffset=0pt,shadow yoffset=0pt,shadow color=green]
      at (\i*45:1) {Circle \i};
\end{tikzpicture}
\end{codeexample}

  Another interesting effect is to set the shadow color to the same
  color as the node itself, causing a ``fuzzy'' look.

\begin{codeexample}[]
\begin{tikzpicture}
  \foreach \i in {1,...,8}
    \node[circle with shadow,fill=red!\i0,shadow scale=1.15,
          shadow xoffset=0pt,shadow yoffset=0pt,shadow color=red!\i0]
      at (\i*45:1) {Red \i0};
\end{tikzpicture}
\end{codeexample}
\end{shape}
