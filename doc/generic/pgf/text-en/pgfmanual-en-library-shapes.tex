...
\subsection{Logic Shapes}

\begin{pgflibrary}{shapes.logic}
  This library provides traditional logic gate shapes.
Multiple inputs can be specified for a logic gate (provided they
support multiple inputs: the |NOT gate|, for example, does not),
however, there is an upper limit for the number of inputs which has 
been (arbitrarily) set at 1024 (!), which should be way more than is
required for most purposes. 

The appearance of the shapes is broadly based on the logic symbols
used in A. Croft, R. Davidson, and M. Hargreaves (1992), 
\emph{Engineering Mathematics}, Addison-Wesley, 82--95.
\end{pgflibrary}

There are three \pgfname{} keys than can be used with the logic 
gates:
...
\begin{key}{/pgf/logic gate inputs=\meta{input list} (initially \char`\{normal,normal\char`\})}
  ...
  circle attached to the main shape of the logic gate. Anything
  that is not the word |inverted| will be treated as a ``normal'' or 
  ...
  up appropriately. In addition, using some special magic, the
  input anchors can also be used to position the node.
  
\begin{codeexample}[]
\begin{tikzpicture}[minimum height=0.75cm]
  \node[AND gate, draw,logic gate inputs={inverted, non-inverted, inverted}] 
  (A) {};
  \foreach \a in {1,...,3}
...
\end{codeexample} 
  ...
  list, thus, a ``shorthand'' specification is permitted (this is an
...
\begin{codeexample}[]
\begin{tikzpicture}[minimum height=0.75cm]
  \node[OR gate, draw,logic gate inputs=inini] (A) {};
  \foreach \a in {1,...,5}
    \draw (A.input \a -| -1,0) -- (A.input \a);
\end{tikzpicture}
\end{codeexample} 
...
\begin{key}{/pgf/logic gate inverted radius=\meta{length} (initially 2pt)}
  ...
  The value of \meta{length} may also influence
  the overall size of the shape as described below.
  
\begin{codeexample}[]
\begin{tikzpicture}[minimum height=0.75cm]
  \tikzset{every node/.style={shape=NAND gate, draw, logic gate inputs=ii}}
...
As the above example shows, \pgfname{} will increase the size of the 
logic gate to accommodate the number of inputs, and the size
of the inverted radius. Any increase in size (including any minimum 
size requirements) will be applied so that the
default aspect ratio is unaltered. This means that changing the
height will change the width and vice versa. 

The ``compass point'' anchors apply to the main part of the shape
and do not include any inverted inputs or outputs. An additonal
feature is also implemented to facilitate the relative positioning
of logic gates:
...
main shape (again, ignoring any inverted inputs or outputs). 
...
\begin{tikzpicture}[minimum height=1.5cm]
  \node[XNOR gate, draw, gray!50,line width=2pt] (A) {};
...
\Huge
\begin{tikzpicture}
  \node[name=s,shape=AND gate,shape example, inner xsep=1.5cm] {AND gate\vrule width1pt height2cm};
  \foreach \anchor/\placement in
...
\Huge
\begin{tikzpicture}
  \node[name=s,shape=NAND gate,shape example, inner xsep=1cm,
  logic gate inverted radius=.65cm] {NAND gate\vrule width1pt height2cm};
  \foreach \anchor/\placement in
    {center/above, text/above, 30/above right,
     mid/right, mid east/left, mid west/above,
     base/below, base east/below, base west/left,
...
\begin{shape}{OR gate}
	...
	less than two inputs are specified an error will result. 
	The anchors for an |OR gate| with two
  non-inverted inputs (using the normal compass point anchors) are
  shown below. Anchor |30| is an example of a border anchor.
...
\Huge
\begin{tikzpicture}
  \node[name=s,shape=OR gate,shape example, inner xsep=1.5cm] {OR gate\vrule width1pt height2cm};
  \foreach \anchor/\placement in
    {center/above, text/above, 30/above right,
     mid/right, mid east/left, mid west/above,
     base/below, base east/below, base west/below,
...
\Huge
\begin{tikzpicture}
  \node[name=s,shape=NOR gate,shape example, inner xsep=1.5cm,
  logic gate inverted radius=.65cm] {NOR gate\vrule width1pt height2cm};
  \foreach \anchor/\placement in
    {center/above, text/above, 30/above right,
     mid/right, mid east/left, mid west/above,
     base/below, base east/below, base west/below,
...
\begin{shape}{XOR gate}
	...
	The anchors for an |XOR gate| with two
  non-inverted inputs (using the normal compass point anchors) are
  shown below. Anchor |30| is an example of a border anchor.
...
\Huge
\begin{tikzpicture}
  \node[name=s,shape=XOR gate,shape example, inner xsep=1.5cm] {XOR gate\vrule width1pt height2cm};
  \foreach \anchor/\placement in
    {center/above, text/above, 30/above right,
     mid/right, mid east/left, mid west/above,
     base/below, base east/below, base west/left,
...
\Huge
\begin{tikzpicture}
  \node[name=s,shape=XNOR gate,shape example, inner xsep=0.5cm,
  logic gate inverted radius=.65cm] {XNOR gate\vrule width1pt height2cm};
  \foreach \anchor/\placement in
    {center/above, text/above, 30/above right,
     mid/right, mid east/left, mid west/above,
     base/below, base east/below, base west/right,
     north/above, south/below, east/above, west/above,
     north east/above, south east/below, south west/below, north west/above,
...
\begin{shape}{NOT gate}
	This shape is an NOT gate, which supports only one inputs. If
	no inputs are specified an error will result. If more
	than one input is specified, the extra inputs are ignored.
	The anchors for an |NOT gate| with two
  non-inverted inputs (using the normal compass point anchors) are
  shown below. Anchor |30| is an example of a border anchor.
...
\Huge
\begin{tikzpicture}
  \node[name=s,shape=NOT gate,shape example, inner sep=1.5cm,
  logic gate inverted radius=.75cm] 
