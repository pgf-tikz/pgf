...
\subsection{The Shapes}
	...
	The shapes that are currently defined are shown below. Some of these
	are available without loading any shape libraries. Many can 
	be more precisely configured using specialised keys (e.g.,
	the number of sides for the regular polygon can be specified, 
	as can the internal angles of the trapezium, or the apex angle
...
\def\splitshapename@#1 #2 #3\@@{\ifx#2.\relax\hyperlink{shape library:#1}{#1}%
  \else\hyperlink{shape library:#1 #2}{\hbox{\vbox{\halign{\hfil##\hfil\cr#1\cr#2\cr}}}}\fi}

\begin{tikzpicture}
  \tikzset{every cell/.style={every node/.style={draw=red!40!black, line width=1.5pt, fill=red!40,  
    anchor=center, execute at begin node={\color{black}\splitshapename{\tikz@shape}}}}}
  \matrix [matrix of nodes, nodes in empty cells, column sep=.65cm, row sep=.5cm]{%
    |[circle]|             & |[rectangle]|       & |[ellipse]|        &       
    |[diamond]|            & |[regular polygon]| \\
    |[star]|               & |[semicircle]|      & |[trapezium]|      & 
    |[isosceles triangle]| & |[dart]|            \\
    |[kite]|               & |[circular sector]| & |[forbidden sign]| & 
    |[cloud]|              & |[starburst]|       \\
    |[signal]|             & |[tape]|            & |[single arrow]|   &
    |[double arrow]|       & |[circle split]|    \\
    |[rectangle split]|    & |[cross out]|       & |[strike out]|     &
    |[rounded rectangle]|  & |[bevelled rectangle]| \\};
\end{tikzpicture}
\end{codeexample}%
}
...
\begin{tikzpicture}
  ...
  \node[cloud, draw, fill=gray!20, cloud aspect=1] at (1.5,0) {D};
\end{tikzpicture}
\end{codeexample}
	...
	\begin{key}{/pgf/cloud aspect=\meta{value} (initially 1.5)}
	  Set the recommend quotient of the width and height of the cloud.
	  \pgfname{} will regard the value in this key only as a
	  recommendation, which cannot be guaranteed to be followed 
	  exactly. 
	\end{key}
	...
	\pgfname{} will, again, be treating them only as recommended sizes.
...
\Huge
\begin{tikzpicture}
  \node[name=s, shape=cloud, style=shape example, cloud puffs=11, cloud puff arc=120] 
  	{Cloud\vrule width 1pt height 2cm};
  \foreach \anchor/\placement in
	...
	\begin{key}{/pgf/signal from=\meta{direction}\space\opt{and \meta{opposite direction}} (initially nowhere)} 
		...
		\begin{itemize}
			\item \declare{|north|} or \declare{|above|}
				add a pointer to the upper side of the signal.
			\item \declare{|south|} or \declare{|below|}
				add a pointer to the lower side of the signal.
			\item \declare{|east|} or \declare{|right|}
				add a pointer to the right side of the signal.
			\item \declare{|west|} or \declare{|left|}
				add a pointer to the left side of the signal.
			\item \declare{|nowhere|} 
				reset the sides so they have no pointers. When used with 
...
\begin{shape}{single arrow}
	...
	points (provided no other transformations are applied).
...
\begin{tikzpicture}[>=stealth,  
    rotate border/.style={shape border uses incircle, shape border rotate=#1}]
  \node[single arrow, single arrow head extend=.5cm, rotate border=-30,
    draw, gray, minimum height=3cm] (arrow) {};
...
\end{codeexample}
	...
	The anchors for the rounded rectangle are shown below (anchor |10| is
...
\begin{shape}{bevelled rectangle}

	This shape is a rectangle with optionally bevelled corners.
	
\begin{codeexample}[]
\begin{tikzpicture}
  \node[bevelled rectangle, white, fill=red, double=red, draw, very thick]
    {\bf STOP!};
\end{tikzpicture}
\end{codeexample}
...
\begin{key}{/pgf/bevelled rectangle angle=\meta{angle} (initially 45)}
	Set the angle \emph{from the vertical} for the bevel.
	
\begin{codeexample}[]
\begin{tikzpicture}
  \tikzset{every node/.style={bevelled rectangle, draw}}
  \node[bevelled rectangle angle=30] {abc};
  \node[bevelled rectangle angle=60] at (1.5,0) {123};
\end{tikzpicture}
\end{codeexample}
\end{key}

\begin{key}{/pgf/bevelled rectangle xsep=\meta{length} (initially .666ex)}
	Set the distance that the bevel extends horizontally beyond the node 
	contents (which includes the |inner sep|). 
	If \meta{length} is large, such
	that the top and bottom bevelled edges would cross, then 
	\meta{length} is ignored and the bevelled edges are drawn so that
	they meet in the middle.

\begin{codeexample}[]
\begin{tikzpicture}
  \tikzset{every node/.style={bevelled rectangle, draw}}
  \node[bevelled rectangle xsep=2pt] {def};
  \node[bevelled rectangle xsep=2cm] at (1.5,0) {456};
\end{tikzpicture}
\end{codeexample}
	
\end{key}

\begin{key}{/pgf/bevelled rectangle ysep=\meta{length} (initially .666ex)}
	Set the distance that the bevel extends veritcally beyond the node 
	contents. 
	If \meta{length} is large, such that the left and right bevelled 
	edges would cross, then \meta{length} is ignored and the bevelled 
	edges are drawn so that	they meet in the middle.
\end{key}

\begin{key}{/pgf/bevelled rectangle sep=\meta{length} (initially .666ex)}
	Set both the |xsep| and |ysep| simultaneously.
\end{key}

\begin{key}{/pgf/bevelled rectangle corners=\meta{list} (initially bevel all)}
	Specify which corners are bevelled. The corners are identified by 
	...
	\meta{list} are automatically not bevelled. Two additional values
	|bevel all| and |bevel none|, are also permitted.

\begin{codeexample}[]
\begin{tikzpicture}
  \tikzset{every node/.style={bevelled rectangle, draw}}
  \node[bevelled rectangle corners=north west] {ghi};
  \node[bevelled rectangle corners={north east, south east}] at (1.5,0) {789};
\end{tikzpicture}
\end{codeexample}
\end{key}
...
\Huge
\begin{tikzpicture}
  \node[name=s,shape=bevelled rectangle, bevelled rectangle sep=1cm,
        shape example, inner ysep=1cm, inner xsep=1cm] 
    {Bevelled Regtangle\vrule width1pt height2cm};
  \foreach \anchor/\placement in
