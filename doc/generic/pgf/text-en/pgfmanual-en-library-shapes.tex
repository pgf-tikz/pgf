...
\end{codeexample}
	...
	which is the ellipse which passes through the points of the
	puff arcs which are furthest from the center of the cloud.
...
\begin{shape}{rounded rectangle}
	This shape is a rectangle with semicircular arcs at each end.
...
\end{codeexample}
	...
	There are keys to specify how the ends are rounded (to use
...
\begin{key}{/pgf/rounded rectangle left arc=\meta{arc type} (initially convex)}
...
\begin{tikzpicture}
  \matrix[row sep=5pt, every node/.style={draw, rounded rectangle}]{
  	\node[rounded rectangle left arc=concave] {Concave}; \\
  	\node[rounded rectangle left arc=convex]  {Convex};  \\
  	\node[rounded rectangle left arc=none]    {None};    \\};
\end{tikzpicture} 
\end{codeexample}
\end{key}

\begin{key}{/pgf/rounded rectangle right arc=\meta{arc type} (initially convex)}
	Set the style of the rounding for the right side.
\end{key}
