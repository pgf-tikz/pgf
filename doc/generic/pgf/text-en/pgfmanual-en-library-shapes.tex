...
\begin{key}{/pgf/logic gate inverted sep=\meta{length} (initially .125cm)}
...
logic gate to accommodate the number of inputs, the size
...
\begin{key}{/pgf/and gate IEC symbol=\meta{text} (initially \char`\\char\char`\`\char`\\\char`\&)}
  ...
  the |logic gate IEC symbol color| key should be used to change the
  color.
...
\begin{key}{/pgf/or gate IEC symbol=\meta{text} (initially \char`\\char\char`\`\char`\\\char`\&)}
...
\begin{key}{/pgf/nor gate IEC symbol=\meta{text} (initially  \char`\{\char`\$=1\char`\$\char`\})}
...
\begin{key}{/pgf/logic gate IEC symbol color=\meta{color}}
  Set the color for all symbols simultaneously. In \tikzname, when the 
