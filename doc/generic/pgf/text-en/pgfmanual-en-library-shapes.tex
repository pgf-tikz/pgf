...
\begin{pgflibrary}{shapes.arrows}
  This library defines arrows shapes.
...
\begin{codeexample}[]
\begin{tikzpicture}
	\tikzset{box/.style={arrow box, fill=#1, minimum size=.5cm}}
	\draw [help lines] grid(3,2);
	\node[box=blue!50, arrow box arrows={east:1cm}]             at (1,1.5){B};
  \node[box=red!50,  arrow box arrows={east:1cm from center}] at (1,0.5){A};
\end{tikzpicture}
\end{codeexample}
...
\begin{key}{/pgf/arrow box arrows={\ttfamily\char`\{}\meta{list}{\ttfamily\char`\}}}
  ...
  If an item specifies no distance, the previously known distance will 
  be used (at the start of the list this is |0cm|, so the first 
  item in the list should specify a distance). 
...
size, this example must be rotated). If a side is drawn without
an arrow, the anchors for the arrow should be considered unavailable.
They are (unavoidably) defined, but default to the center of the
appropriate side.
...
\begin{tikzpicture}
  ...
  \foreach \anchor/\placement in
   {center/above, text/above, mid/right, base/below,
...
\begin{pgflibrary}{shapes.callout}
  Producing basic callouts is quite simple in \pgfname{} and 
...
the coordinateto this point. This seemingly over-complex approach 
...
absolutely. In addition, by using the 
|remember picture| option in \tikzname{} (or the \TeX-if 
|\ifpgfrememberpicturepositiononpage| in \pgfname), the pointer can 
point to named nodes in other pictures.
...
The following keys are common to both callouts. Please remember
...
\begin{shape}{rectangle callout}%
  ...
  tightly fits the node contents (including any |inner sep|). This
  callout can be used to point to nodes in other pictures. It
  supports the keys described above and also the 
  following key:
...
\begin{shape}{ellipse callout}%
  ...
  tightly fits the node contents (including any |inner sep|), and can 
  point to nodes in other pictures. It supports the keys described above
  for specifying the pointer, and also the following key:
