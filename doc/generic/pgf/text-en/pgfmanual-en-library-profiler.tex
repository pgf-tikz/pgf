...
\begin{command}{\pgfprofilenewforcommand\marg{\textbackslash macro}\marg{arguments}}
	Defines a new profiler entry which will measure the time spent in \meta{\textbackslash macro}. This calls |\pgfprofilenew| and replaces the current definition of \meta{\textbackslash macro} with a new one. The profiler entry will be named \declareandlabel{\pgfprofilecs}\meta{macro} (without backslash) where |\pgfprofilecs| is predefined to be \texttt{\pgfprofilecs}. The replacement macro will collect all required arguments, start counting, invoke the original macro definition and stop counting.
...
\begin{command}{\pgfprofilenewforenvironment\marg{environment name}}
	Defines a new profiler entry which measures time spent in the environment \meta{environment name}. The profiler entry will be called \declareandlabel{\pgfprofileenv}\meta{environment name} where |\pgfprofileenv| is predefined as \texttt{\pgfprofileenv}.
...
\begin{command}{\pgfprofilepostprocess}
	For \LaTeX, this command is installed automatically in |\end{document}|. It stops all running timings, evaluates them and returns the result into the logfile. Furthermore, it generates a text table |\jobname.profiler.dat|.
\end{command}
