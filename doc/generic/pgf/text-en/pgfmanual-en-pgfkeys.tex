...
% calling the key yields 'Initial definition. ':
\pgfkeys{/the/key}
...
\begin{command}{\pgfkeyssetdefaultpathforhandled\marg{key path prefix}}
	Changes the initial configuration how key handlers will be used: Allows to change the default path for handled keys.
...
\begin{handler}{{.search also}=\marg{path list}}
	...
	To also enable searching for styles (or other handled keys), consider changing the configuration for handled keys to |/hander config=full or existing| when you use |.search also|.
\end{handler}
