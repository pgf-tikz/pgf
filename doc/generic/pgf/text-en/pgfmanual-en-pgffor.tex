...
\begin{key}{/pgf/foreach/evaluate=\meta{variable}| |\opt{|as |\meta{macro}| (trim integers)|}}
  By default list items are not evaluated: |1+2|, yields |1+2|, 
  not |3|. This key allows a variable (specified either using the
  |var| key or in the \meta{variables} argument if the |foreach|
  command), to be evaulated using the mathematical engine. 
  The optional |as |\meta{macro} will mean, the evaluation will
  be stored in \meta{macro}, and the |(trim integers)| keyword
...
\begin{key}{/pgf/foreach/remember=\meta{variable}| as |\meta{macro}| |\opt{|(initially |\meta{value}|)|}}
	...
	Optionally, the value of \meta{macro} can be set for the first
...
\begin{key}{/pgf/foreach/count=\meta{macro}| |\opt{|from |\meta{value}}}
  ...
  to begin from \meta{value}. 
  
\begin{codeexample}[]
\begin{tikzpicture}[x=20pt,y=20pt,every node/.style={draw,minimum size=18pt}]
\foreach \x [count=\xi] in {a,...,e}
  \foreach \y [count=\yi] in {a,...,e}
    \node at (\xi,-\yi) {$\mathstrut\x\y$};
\end{tikzpicture}
\end{codeexample}
\end{key}
