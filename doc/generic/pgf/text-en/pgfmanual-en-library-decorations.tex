...
\end{codeexample}
  ...
  There are many keys and styles that be used to add effects
...
\begin{key}{/pgf/decoration/text effects/word variable=\meta{macro}}
   This key stores the number of word in the text
 		\meta{macro}. Numbering starts at 1 and
 		  the character acting as a word separator is numbered 0.
...
\begin{key}{/pgf/decoration/path from text=\opt{\marg{true or false}} (default true)}
...
\begin{key}{/pgf/decoration/path from text angle=\meta{angle}}
...
\begin{key}{/pgf/decoration/text effects/group letters into words}
...
\path [decorate] (0,0);
\path [decorate, text effects={group letters into words,
...
\begin{key}{/pgf/decoration/text effects/replace characters=\meta{characters} with \marg{code}}
  ...
  Replace each node for each  character in \meta{characters} with \meta{code}. 
