...
\begin{decoration}{curve control points}
	This decoration replaces the input path by paths reflecting the 
	control points of each curve in the input path.
	It can only be used in \tikzname, and it is best used as a pre- 
	or post-action. Any input path segment that is not a curve is 
	ignored.
	Also, it is important to remember when drawing arcs greater than 
  90\textdegree, that control points are drawn reflecting
  the way that \tikzname{} draws such arcs. This may be different
  to the way you might be expecting.
  	
\begin{codeexample}[]
\begin{tikzpicture}
  \draw [help lines] grid (3,2);
  \draw [thick, preaction={decoration=curve control points, decorate},
    curve control action/.style={red, draw}] 
    (1,0) .. controls (0,2) and (2.5,2) .. (3,1);
\end{tikzpicture}
\end{codeexample}
  
 There are various \tikzname{} keys to customize how the
 control points are drawn:
  
\begin{stylekey}{/tikz/curve control action (initially draw)}
  Specify actions for the curve control paths.
\begin{codeexample}[]
\tikzset{
  curve control action/.style={draw, blue, dashed, thin,
    postaction={decoration={
      markings,
      mark=at position -1sp with
      {
        \draw [red,solid] (-2pt,-2pt) -- (2pt,2pt);
        \draw [red,solid] (2pt,-2pt) -- (-2pt,2pt);
      }
    },
    decorate}
  ...
  show control points/.style={
    postaction={decoration=curve control points, decorate}
  }
}
\begin{tikzpicture}
  \draw [help lines] grid (3,2);
  \draw [thick, show control points]
    (0,2) .. controls (2.5,1.5) and (0.5,0.5) .. (3,0);
\end{tikzpicture}
\end{codeexample}
\end{stylekey}

\begin{key}{/tikz/create curve control nodes=\opt{\meta{boolean}} (default true)}
  In addition to creating the paths, \tikzname{} can create named
  coordinate nodes, one for each point on the curve. The names take
  the form \meta{prefix}-1, \meta{prefix}-2, \meta{prefix}-3, 
  \meta{prefix}-4, corresponding to the first point, first support 
  point, second support point, and last point of the curve, 
  respectively:

\begin{codeexample}[]
\begin{tikzpicture}[curve control action/.style={draw, blue, dashed, thin}]
  \draw [help lines] grid (3,2);
  \draw [thick, create curve control nodes]
    [postaction={decoration=curve control points, decorate}]
    (0,2) .. controls (2.5,1.5) and (0.5,0.5) .. (3,0);
  \foreach \i in {1,2,3,4}
    \node [above] at (curve point-\i) {$P_{\i}$};
\end{tikzpicture}
\end{codeexample}

  If the input path contains more than one curve segment then subsequent
  nodes are labelled \meta{prefix}-5 (for the first point on the
  second curve), \meta{prefix}-6 (for the first support point on the
  second curve), and so on.
  As the counter for the control points is reset before the decoration 
  is run, the prefix can be changed between each use of the decoration 
  using the following key:

\end{key}

\begin{key}{/tikz/curve control node prefix=\meta{text} (initially curve point)}
  Change the prefix to \meta{text} for all curve points on the input path. 
  Using this key implies that the curve control nodes should be created,
  so the |create curve control nodes| key can be omitted.
\end{key}

\begin{key}{/tikz/show curve control polygon=\opt{\meta{boolean}} (default true)}
  When \meta{boolean} is |true|, each input path curve segment will be 
  replaced by a path that follows the polygon of the curve 
  controls (omitting the chord --- the line between the start and end 
  points of the curve).

\begin{codeexample}[]
\begin{tikzpicture}[curve control action/.style={draw, blue, dashed, thin}]
  \draw [help lines] grid (3,2);
  \draw [thick, show curve control polygon]
    [postaction={decoration=curve control points, decorate}]
    (0,0) .. controls (0.5,1.75) and (2.5,1.75) .. (3,0);
\end{tikzpicture}
\end{codeexample}
