...
\begin{key}{/tikz/graphs/use existing nodes=\opt{\meta{true or false}} (default true)}
    ...
    and you now only wish to connect the nodes. It also implies that
...
\end{codeexample}
    ...
    \end{key}
    %
    % To avoid showing the double dash as en-dash there is a zero-width space
    % between the dashes (see bug #464). This doesn't seem to be present in the
    % PDF and thus copy--pasting from the PDF doesn't result in any problems.
