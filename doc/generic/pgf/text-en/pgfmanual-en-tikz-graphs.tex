...
\tikz [shorten >=1pt, rotate=90, xscale=-1, mark/.style={fill=black!50}, mark/.default=]
   \graph [trie, simple, sibling distance=8mm, nodes={circle,draw},
           edges={nodes={inner sep=1pt,anchor=mid,fill=graphicbackground}},
           put node text on incoming edges, layered layout]
    {
      root[mark] -> {
        a -> n -> {
          g [mark],
...
a slash or, alternatively, after double underscores:

\begin{codeexample}[]
\tikz \graph {
  x1/$x_1$ -> x2 [as=$x_2$, red] -> x3__$x_3$;
  x1 -> [bend left] x3;
};  
\end{codeexample}
...
specification. There are four different possible kinds of node
...
\item[Direct Node Specification]
  ...
  \meta{node name}\opt{|/|\meta{text}} \opt{\oarg{options}}
\item[Quoted Direct Node Specification]
  \ \\
  |"|\meta{text}|"|\opt{\oarg{options}}\\
...
with an opening curly brace, a group specification is meant; if it
starts with a quotation mark (which may, but need not be an active
character), it is a quoted direct node specification; and in
all other cases a direct node specification is meant.

\medskip
\textbf{Direct Node Specifications.} If after reading the first symbol
of a node specification is has been detected to be \emph{direct} (but
not quoted),
\tikzname\ will collect all text up to the next edge
specification and store it as the \meta{node name}; however, square
brackets are used to indicate options and both a double underscore
and also a slash end the \meta{node name} and start a special
\meta{text} that is used as a 
...
semicolons, hyphens, braces, dots, parentheses, slashes, dashes, and
...
\item
  ...
  is no \meta{node name} before the two underscores or before the
  slash), then a fresh internal node name is created and used as
  ...
  starting with a double underscore or with a slash represents an
...
\tikz \graph { a [as=$x$] -- b [as=$y_5$] -> c [red, as={a--b}] };
  ...
  contains a double underscore or a slash, the text to the right of the
  double underscore or the slash is stored in the macro |\tikzgraphnodetext|; if
  there is no double underscore and no slash, the \meta{node name} is stored in
  ...
  \begin{key}{/tikz/graphs/typeset=\meta{code}}
    ...
    \begin{command}{\tikzgraphnodetext}
      ...
      is neither a double underscore nor a slash, to the \meta{node name}.
    \end{command}
...
\medskip
\textbf{Quoted Direct Node Specifications.} A quoted direct node
specification is detected by the fact that the first symbol of a node
specification is a quotation mark (|"|). Before we have a detailed
look at what happens in this case, here is the short version: When you
surround a node text by quotation mark, you can use all sorts of
special symbols as part of the node name that are normally
forbidden. These special symbols will be internally replaced by their
Unicode names:
\begin{codeexample}[]
\tikz \graph [grow right=4cm] {
  "Hello, World!" -> "This is \emph{important}!";
  normal node -> "This is \emph{important}!";
};
\end{codeexample}
You can also use options by placing them behind the last quotation
mark:
\begin{codeexample}[]
\tikz \graph [grow right=4cm] {
  "Hello, World!"[red] -> "This is \emph{important}!"[scale=.5];
%  normal node -> "Hello, World!";
};
\end{codeexample}

In detail, the following happens when a node specification starts with
a quotation mark:
\begin{itemize}
\item Everthing following the quotation mark up to the next single
  quotation mark is collected into \meta{text}. All sorts
  of special characters, including commas, square brackets, dashes,
  and even backslashes are allowed here. Basically, the only
  restriciton is that braces must be balanced.
\item A double quotation mark (|""|) also does not count as the ``next
  single quotation mark.'' Rather, it is replaced by a single
  quotation mark. For instance, |"He said, ""Hello world."""| would be
  stored inside \meta{text} as |He said, "Hello world."|
  However, this rule applies only on the outer-most level of
  braces. Thus, in
\begin{codeexample}[code only]
"He {said, ""Hello world.""}"
\end{codeexample}
  we would get |He {said, ""Hello world.""}| as \meta{text}.
...
Now, once the \meta{text} has been collected, a normal direct node is
created as if had written
\begin{quote}
  \meta{escaped text}|/{|\meta{text}|}|\opt{\oarg{options}}
\end{quote}
Here, the \meta{escaped text} is obtained from the \meta{text} as
follows: Every ``special character'' in the \meta{text} is replaced by
its Unicode name. For instance, if \meta{text} is |Hello, world!|, the
\meta{escaped text} is |HelloCOMMA worldEXCLAMATION MARK|, which will
be the name of the node. The ``special characters'' are the following:
Every Unicode character with a decimal code number between 33 and 126
that is neither a digit nor a letter.
...
\begin{key}{/tikz/graphs/grid placement}
  ...
  \begin{codeexample}[]
% An example with 6 nodes, 3 colums and therefor 2 rows
