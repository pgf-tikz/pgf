...
In a ``mathematical'' graph, each edge has a ``tail'' and a
...
\begin{key}{/tikz/rdf engine/statement=\marg{options}}
  ...
  output file. The \meta{options} will be executed with the path
...
\end{codeexample}
 ...
 \begin{key}{/tikz/rdf engine/statements/subject=\meta{subject}}
    ...
    \meta{subject} can be in one of two possible formats:
    \begin{enumerate}
    ...
    \item When the \meta{subject} starts with an opening parenthesis,
      ...
      |(|\meta{node or scope name}|)|. In this case, the \meta{node or
        scope name} must be the name of an already existing node (the
      current node or scope is considered as ``existing'' here). Then,
      the curie |#|\meta{id} is used as subject,
      where the \meta{id} is a unique internal identifier for the
      node.
...
\end{codeexample}
    \end{enumerate}
    You can use a macro as \meta{subject}, it will be expanded before
  ...
  \begin{key}{/tikz/rdf engine/statements/object=\meta{object}}
    ...
    \item As for |subject| and |predicate|, you can use the syntax
      |(|\meta{name of node or scope}|)| to create and use a curie for
      the node or scope.
    \item If the \meta{object} starts with |"|, it must have the
      syntax |"|\meta{literals}|"|. In this case, the object of the
    ...
    \item The two previous cases can be combined in the form of an
      ...
      but this gets ``overruled'' by the \meta{literals}. Formally,
      this means that the object is the \meta{literals}, but the
      intended semantics is that the object is the scope content, only
      for further processing it should be considered to be
      \meta{literals}. A typical example is the case where the scope
...
\begin{key}{/tikz/rdf engine/get scope curie=\meta{macro}}
  The \meta{macro} will be set to a unique curie that represents the
...
\begin{key}{/tikz/rdf engine/statements/has as member}
  ...
  increased and the predicate will be set to |rdf:_|\meta{count}. This
...
Let us change the different keys so that they add \textsc{rdf}
...
The first statement tells us that the circle with its contents is a
...
The style for |initial| is the same as for |final| only with a
different type.
...
In case automata contained \emph{only} states, we could setup the
...
\emph{Using} this code is still ``as easy as before,'' indeed, the
code for creating the automaton is perfectly unchanged:
...
Let us now have a look at the result. If the above is processed using
\TeX\ and transformed to \textsc{svg} code, the following results
(reformatted and slightly simplified):
