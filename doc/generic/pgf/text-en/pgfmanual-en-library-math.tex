...
\begin{codeexample}[]
\tikzmath{
  % Adpated from http://www.cs.northwestern.edu/academics/courses/110/html/fib_rec.html
  function fibonacci(\n) {
    if \n == 0 then {
      return 0;
    } else {
...
\meta{additional variables}}\texttt{;}}
	The |real| keyword ensures that assginments \meta{variable}
	(and \meta{additional variables}) will not be trunctaed to integers.
\end{math-keyword}
...
\begin{math-keyword}{{if} \meta{condition} \texttt{then \{}\meta{if-non-zero-statments}\texttt{\};}}

	This keyword executes \meta{if-non-zero-statments} if the expression in \meta{condition}
	evaluates to any value other than zero.

\end{math-keyword}

\begin{math-keyword}{{if} \meta{condition} \texttt{then \{}\meta{if-non-zero-statments}\texttt{\}} \texttt{else} \texttt{\{}\meta{if-zero-statments}\texttt{\}}\texttt{;}}

	This keyword executes \meta{if-non-zero-statments} if the expression in \meta{condition}
	evaluates to any value other than zero and the \meta{if-zero-statments} are executed if
