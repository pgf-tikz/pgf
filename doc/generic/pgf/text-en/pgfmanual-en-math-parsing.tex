...
The following functions allow the dimensions of (typically) text to
be used in the parser. Note that, as the parser uses |\edef| to expand
an expression before parsing commences, the argument to these functions
should be simple text. Also, you should remember that even though the
parser measures the dimensions in points, the value returned has no
units.


\begin{math-function}{widthof(\mvar{text})}

	The width of \mvar{text} in points. 

\begin{codeexample}[]
\pgfmathparse{widthof(Simple text)} \pgfmathresult
\end{codeexample}

\end{math-function}

\begin{math-function}{heightof(\mvar{text})}

	The height of \mvar{text} in points. 

\begin{codeexample}[]
\pgfmathparse{heightof(Simple text)} \pgfmathresult
\end{codeexample}

\end{math-function}

\begin{math-function}{depthof(\mvar{text})}

	The depth of \mvar{text} in points. 

\begin{codeexample}[]
\pgfmathparse{depthof(Simple text)} \pgfmathresult
\end{codeexample}
