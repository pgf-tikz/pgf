...
\end{codeexample}
  ...
  Unlike the logical gates, there are no default |input| and |output|
  anchor with ee-symbols. This may change in the future. The node
  are currently only guaranteed to have the standard compass direction
  anchors.
...
\begin{pgflibrary}{shapes.gates.ee}
  This library defines a shape called |direction ee| and an arrow tip,
  also called |direction ee|.
\end{pgflibrary}
...
arrow options, which should be set to the desired size.

\begin{shape}{direction ee}
  This shape is rather special. 
\end{shape}
