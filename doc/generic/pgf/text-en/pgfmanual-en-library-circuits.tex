...
\begin{tikzlibrary}{circuits.ee}
  ...
  The library also declare some standard annotations. Unlike 
\end{tikzlibrary}
...
\begin{key}{/tikz/circuit ee IEC}
  ...
  graphics for the logical symbols like |resistor|. When
  you include this library, you can use the following key to 
  setup a scope that contains a circuit:


    This key calls |circuit ee| and installs the IEC-like
    graphics. As explained in Section~\ref{section-theming-symbols},
    for each graphic symbol of the library there is also a style that
    stores this  particular appearance.
    These keys are called |resistor IEC graphic|,
    |or gate IEC graphic|, and so on.
\begin{codeexample}[]
\begin{tikzpicture}[circuit logic IEC,
                    every circuit symbol/.style={
...
been produced by 
|\draw (0,0) to[|\meta{symbol name}|] (3,0);| for the symbols that lie
``on'' a line and by |\draw (0,0) to[|\meta{symbol name}|=at end] (1.5,0);|
...
\begin{codeexample}[]
\begin{tikzpicture}[circuit ee IEC]
  \draw (0,5) to[diode          = light emitting]  (3,5);
  \draw (0,4) to[diode          = light dependent] (3,4);
  \draw (0,3) to[resistor       = light dependent] (3,3);
  \draw (0,2) to[battery        = light dependent] (3,2);
  \draw (0,1) to[voltage source = direction info]  (3,1);
\end{tikzpicture}
\end{codeexample}
