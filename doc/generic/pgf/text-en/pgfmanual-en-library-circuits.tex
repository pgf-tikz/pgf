...
Here is an example:
\begin{codeexample}[]
\begin{tikzpicture}[circuit ee IEC,resistor graphic=var resistor IEC graphic,x=2cm,y=3cm]
  % Let us start with some links:
  \foreach \x in {1,...,5}
  {
    \node [link] (upper link \x) at (\x,1) {};
    \node [link] (lower link \x) at (\x,0) {};
  }
  \draw (lower link 1) -- (lower link 2) -- (lower link 3)
     -- (lower link 4) -- (lower link 5);
  
  \draw (upper link 1) to [diode] ++(left:1)
                       to [voltage source={near start,volt=3},
                           resistor={near end,ohm=3}] ++(down:1)
                       to (lower link 1);
  \draw (upper link 1) to [resistor={ohm=4}] (lower link 1);
  \draw (upper link 1) to [resistor={ohm=3}] (upper link 2);
  \draw (upper link 2) to [voltage source={near start,volt=8},
                           resistor={ohm=2,near end}] (lower link 2);
  \draw (upper link 2) to [resistor={ohm=1}] (upper link 3);
  \draw (upper link 3) to [resistor={info={$R=4\Omega$}}] (lower link 3);
  \draw (upper link 4) to [voltage source={near start,volt=8},
                           resistor={ohm=2,near end}] (lower link 4);
  \draw (upper link 3) to [resistor={ohm=1}] (upper link 4);
  \draw (upper link 4) to [resistor={ohm=3}] (upper link 5);
  \draw (upper link 5) to [resistor={ohm=4}] (lower link 5);
  \draw (upper link 5) to [diode] ++(right:1)
                       to [voltage source={near start,volt=3},
                           resistor={near end,ohm=3}] ++(down:1)
                       to (lower link 5);
\end{tikzpicture}
