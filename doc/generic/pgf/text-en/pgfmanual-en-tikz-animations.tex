...
\begin{tikzpicture}[
    orbit/.style 2 args = { animate me = {
      :drive = { along = {(0,0) circle [radius=#1]},
                 along in = #2/10,
                 repeats }}} ]

  \node [animate me = { :color = { 0s = "orange",
                                   2s = "red",
                                   4s = "orange",
                                   repeats }}
        ] {Sun};

  \begin{scope}[orbit = {2.5cm}{365}]
    \node {Earth};
    \node [orbit = {1cm}{28}] {Moon};
  \end{scope}
...
To make a long story short: \tikzname\ animations currently work only
...
Two basic limitations of the whole animation system should be pointed
out right here, at the beginning:
\begin{enumerate}
\item 
  It is
  extremely difficult to animate ``lines between moving nodes''
  correctly. Consider code like |\draw(a)--(b);| where |a| and |b| are
  nodes. It is easy to move around |a| or |b| with a minimal amount of
  code, and it is also easy to move the connecting line along when both
  nodes move simultaneously. However, when only |a| moves, you would
  like the connecting line between |a| and |b| to ``follow,'' but
  achieving this involves a huge amount of code if it is possible at
  all.
\item
  Related to the first points, but also a general problem is the fact
  that it is current also next to impossible to animate \emph{arrow
...
\begin{key}{/tikz/animate=\meta{animation specification}}
  ...
  |animate|. You can, and usually should, place the specification of
  all timelines of a single picture inside a single use of this key
  since it will reset the time and the fork time (explained in
  Section~\ref{section-anim-fork}). You can, however, use this key
  several times, in principle. Note that if you animate the same
  attribute of the same object in two different uses of |animate|, two
  separate timelines will result (and complicted rules are used to
  determine which one ``wins'' in case they specify conflicting values
  for the attribute at different times).
...
\tikz \node [fill, text = white, animate = {
  myself:fill = {0s = "red", 2s = "blue", begin on = click }}] {Some Text};
\end{codeexample}
\begin{codeexample}[animation list = {0.5,1,1.5,2}]
...
\end{codeexample}
  ...
  The details of what, exactly, happens in the \meta{animation
    specification} will be described in the rest of this
  ...
  syntax handlers installed. In particular, you can define styles for
  this key path and use them. For instance, we can define a |shake|
  animation like this:
\begin{codeexample}[width=3.2cm]
\tikzset{
  animate/shake/.style = {:xshift = { begin on=click,
      0s = "0mm", 50ms = "#1", 150ms = "-#1", 250ms = "#1", 300ms = "0mm" }}}
\tikz \node [fill = blue!20, draw=blue, very thick, circle,
  animate me = {shake = 1mm}] {Shake};
\tikz \node [fill = blue!20, draw=blue, very thick, circle,
  animate me = {shake = 2mm}] {SHAKE};
\end{codeexample}
...
\item |id| for selecting the timeline id (explained in
  Section~\ref{section-anim-def-ids}),
...
\begin{key}{/tikz/animate/entry}
  ...
  special options that influence the timeline (like |begin on|) are
  also to the timeline of the  object--attribute pair, but see
  Section~\ref{section-anim-options} for details.
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz [animate = {
...
\tikz [animate = {
  ...
  \node (node) [fill, text=white] { Click me };
\end{codeexample}
  Note, however, that in both example we actually add the time--value
...
In the following, we have a closer look at the five keys the influence
...
\begin{key}{/tikz/animate/object=\meta{list of objects}}
  The \meta{list of objects} is a comma-separated list of strings of
  the form \meta{object}\opt{|.|\meta{type}}. All of the objects in
  the list are selected as to-be-animate object for the next use of
  the |entry| key. The objects referred to by \meta{object} will be
  the \emph{next} objects with the |name| 
  key set to \meta{object}. You can apply the |name| key to nodes
...
\end{codeexample}
  ...
  If the \meta{object} name is never used later in the file, no
  animation is created. 

  The \meta{object} may also be the special text |myself|. In this
  ...
  The \meta{object} may be followed by a dot and a \emph{type}. This
...
\begin{key}{/tikz/animate/id=\meta{id} (initally default)}
...
\begin{key}{/tikz/animate/time=\meta{time}\opt{|later|}}
  ...
  \meta{time} plus the current fork time. The text |later|
  ...
  result is interpreted as a time in seconds. Thus, a \meta{time} of |2+3|
  means ``5 seconds'' and a \meta{time} of |2*(2.1)| means ``4.2
  ...
  \meta{time} is interpreted as an offset to the time in the previous
  use of the time key:

\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz \node [animate me = {:fill = { begin on = click,
    0s = "white",
    500ms later = "red",
...
\end{codeexample}
  ...
  The time meant by the value \meta{time} passed to the |time| key is
...
\tikz [animate/highlight/.style = {
  ...
  \node [animate me = { :fill = { 0s = "black", begin on = click },
...
\end{codeexample}
  ...
  \begin{key}{/tikz/animate/remember=\meta{macroname}}
    ...
    |time| key) globally in the macro \meta{macroname}. This time will include
...
\end{codeexample}
  ...
  \begin{key}{/tikz/animate/resume=\meta{absolute time}}
    The \meta{absolute time} is evaluated using |\pgfparsetime| and,
    then, the current time is set to the resulting time minus the fork
    time. When the \meta{absolute time} is a macro previously set
...
\begin{key}{/tikz/animate/value=\meta{value}}
  ...
  \meta{value}. This means that the value of the point in the timeline
...
\begin{key}{/tikz/animate/sync=\meta{options}}
  A shorthand for |scope={| \meta{options}
...
Inside the \meta{animation specification} passed to the |animate| key,
...
timeline (see Section~\ref{section-anim-ids}). 
...
used inside the \meta{options} will be available afterwards as the
last time. Also note that an |entry| is added at the end, so any
settings of keys like |begin| or |repeats| inside the \meta{options}
...
\meta{object}, which is interpreted as the scope or node where the
...
Here, \meta{time} is a text that ``looks like a time,'' which means
that:
\begin{enumerate}
...
mark\footnote{Of catcode 12 for those knowledgeable of such
  things.} (and note that when the \meta{value} contains a comma, you have to surround
...
The following \meta{attributes} are permissible (actually, the
...
These attributes are detailed in the following sections, but here is a
...
key):  
\begin{itemize}
\item |:motion| allows you to specify a path along which the object
  should be moved (using the |along| key). The values given to the
  |entry| key for this attribute refer to a \emph{fraction of the
    distance along the path}. See the |along| key for details.
...
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz [animate me = {:fill = {0s = "red", 2s = "blue",
                              begin on = click}},
...
\tikz [my anim/.style={
         animate me = {
           :text = {0s = "red", 2s = "blue", begin on = click}}},
       text = white, fill = orange ] {
...
\end{codeexample}
  ...
  color for scopes: 
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz [animate me = {:text = {0s = "red", 2s = "blue",
                              begin on = click}},
       text = white, fill = orange ] {
  \node [fill] at (0,0) {A};
  \node [fill] at (1,0) {B};
...
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz \node [
    animate me = { :fill opacity = { 0s="1", 2s="0", begin on=click}},
...
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz \node [
    animate me = { :opacity = { 0s="1", 2s="0", begin on=click}},
    fill = blue!20, draw = blue, ultra thick, circle
  ] {Click me!}; 
\end{codeexample}
\end{tikzanimateattribute}
...
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz \node [
    animate me = { :line width = { 0s="1pt", 2s="5mm", begin on=click}},
    fill = blue!20, draw = blue, ultra thick, circle
  ] {Click me!}; 
\end{codeexample}
...
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz \node [
    animate me = { :dash = { 0s="on 10pt off 1pt  phase 0pt",
...
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz \node [
    animate me = { :dash = { 0s="on 1cm off 1pt phase 0pt",
...
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz \node [ 
    animate me = { :dash phase = { 0s="0pt", 2s="1cm", begin on=click}},
    fill = blue!20, draw = blue, ultra thick, circle, dashed
  ] {Click me!}; 
\end{codeexample}
\end{tikzanimateattribute}
...
matrix. (All keys that animate the transformation matrix \emph{always}
accumulate.)
...
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz \node [
    animate me = { :scale = { 0s="1", 2s="0.2", begin on=click}},
    fill = blue!20, draw = blue, ultra thick, circle
  ] {Click me!}; 
\end{codeexample}
\end{tikzanimateattribute}
...
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz \node [
    animate me = { :rotate = { 0s="45", 2s="90", begin on=click}},
    fill = blue!20, draw = blue, ultra thick, circle
  ] {Click me!}; 
\end{codeexample}
...
\begin{animateattribute}{xskew, yskew, xslant, yslant}
  The keys add an animation of the skew (given in degrees) or slant
  (given as in the |xslant| and |yslant| key):
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz \node [
    animate me = { :xskew = { 0s="0", 2s="45", begin on=click}},
    fill = blue!20, draw = blue, ultra thick, circle
  ] {Click me!}; 
\end{codeexample}
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz \node [
    animate me = { :xslant = { 0s="-1", 2s="1", begin on=click}},
    fill = blue!20, draw = blue, ultra thick, circle
  ] {Click me!}; 
\end{codeexample}
...
\begin{animateattribute}{shift, xshift, yshift}
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz \node [
    animate me = { :shift = { 0s="{(0,0)}", 2s="{(5mm,-5mm)}",
                              begin on=click}},
    fill = blue!20, draw = blue, ultra thick, circle
  ] {Click me!}; 
\end{codeexample}
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz \node [
    animate me = { :xshift = { 0s="0pt", 2s="5mm", begin on=click}},
    fill = blue!20, draw = blue, ultra thick, circle
  ] {Click me!}; 
\end{codeexample}
\end{animateattribute}

Next, there are two attributes that allow you to specify a movement
differently: 

\begin{tikzanimateattribute}{motion, drive}
  The |:motion| attribute lets you move the object ``along a path.'' That
  means that you use the |along| key, explained in a moment, to
  specify a path and the animation will smoothly move the object along
  this path. Note that the path is \emph{not} a value (and no morphing
  occurs); rather, the values are fractions of the distance along the
  path. A value of |"0"| refers to the beginning of the path and |"1"|
  refers to the end:
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz \node [
    animate me = { :motion = {
        along = { (0,0) circle [radius=5mm] },
        0s="0", 2s="1", begin on=click} },
    fill = blue!20, draw = blue, ultra thick, circle
  ] {Click me!}; 
\end{codeexample}

  The path along which you move the object is specified using the
  following key:
  \begin{key}{/tikz/animate/options/along=\meta{path}}
    This key must be used with both the |:motion| and the |:drive|
    attributes to 
  ...
  In most motion animations, you will set the value for |0s| to |"0"|
  and the value for some specific time to |"1"|. The following key
  makes this a bit more convenient:
  \begin{key}{/tikz/animate/options/along in=\meta{time}}
    This is a shorthand for |scope={0s="0", |\meta{time}|="1"}|.    
  \end{key}

  The |:drive| attribute works exactly the same way as the |:motion|
  attribute, only there is an additional rotation that varies in such
  a way that a tangent to the path always points right.  
...
\subsubsection{Setting Up the Animation Transformation Coordinate System}

For the above animations that install an additional canvas
transformation, it is not immediately clear, which coordinate system
should be used for this animation. That is, when you move an object
1cm ``to the right,'' where is ``the right''?

The coordinate system used for determining where the origin of a
rotation is or in which direction a movement ``to the right'' will
actually be is the coordinate system installed for the to-be-animated
object. This is the reason why in the following example the red box
gets rotated its center, whereas the blue box gets rotate around its
lower left corner:

\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz [animate me = { :rotate = { 0s="0", 2s="90", begin on=click} }]
  \fill [fill = blue!20, draw = blue, ultra thick] (0,0) rectangle (1.5,1);
\end{codeexample}
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz [shift = {(7.5mm,5mm)}]
  \scoped [animate me = { :rotate = { 0s="0", 2s="90", begin on=click}}]
    \scoped [shift = {(-7.5mm,-5mm)}]
      \fill [fill = red!20, draw = red, ultra thick] (0,0) rectangle (1.5,1);
\end{codeexample}



The above sequence of first shifting, then animating, and then
shifting back is often needed
...
\begin{codeexample}[animation list={0.5,1,1.5,2}]
\tikz \node [
    animate me = { :rotate = { 0s="45", 2s="90",
                               origin={(0mm,-5mm)}, begin on=click}},
    fill = blue!20, draw = blue, ultra thick, circle
  ] {Click me!}; 
\end{codeexample}
\end{key}
...
\begin{key}{/tikz/animate/forever}
  To be written
\end{key}

\begin{key}{/tikz/animate/options/restart}
  To be written
\end{key}
...
\begin{key}{/tikz/animate/repeats}
  To be written
\end{key}

\subsubsection{Smoothing and Jumping Timelines}

\begin{key}{/tikz/animate/jump}
  To be written
\end{key}

\begin{key}{/tikz/animate/stay}
  To be written
\end{key}

\begin{key}{/tikz/animate/ease}
  To be written
\end{key}

\begin{key}{/tikz/animate/ease in}
  To be written
\end{key}

\begin{key}{/tikz/animate/ease out}
  To be written
\end{key}

\begin{key}{/tikz/animate/entry control}
  To be written
\end{key}

\begin{key}{/tikz/animate/exit control}
  To be written
\end{key}
...
\begin{key}{/tikz/make snapshot of=\meta{time}}
  ...
  have at the specified \meta{time} and inserts the necessary system
...
\tikz [make snapshot of = 1s] {
  \fill [animate me = { :fill = { 0s = "black", 2s = "white" } }]
    (0,0) rectangle ++(1,1);
  \fill [animate me = { :fill = { 1s = "black", 3s = "white" } }]
    (2,0) rectangle ++(1,1);
}
\end{codeexample}

  The moment \meta{time} is best thought of as \meta{time} seconds
  ...
  \begin{key}{/tikz/animate/options/begin snapshot=\meta{start time}}
    ...
    taking snapshots, the timeline starts at \meta{start time} rather
    than at ``moment zero.'' (Think of this as saying that the
    animation starts when a virtual user clicks on the animation and
    this click occurs \meta{start time} seconds after the general
    ``moment zero,'' causing the animation to ``lag behind'' by this
    amount of time.) Computationally, for the timeline the \meta{start
      time} is subtracted from the snapshot's \meta{time} when the
...
\begin{key}{/tikz/make snapshot after=\meta{time}}
  Works exactly like |make snapshot of|, only the \meta{time} is
...
\begin{key}{/tikz/make snapshot if necessary=\meta{time} (default 0s)}
  This key makes a snapshot of \meta{time} only when the output format
