...
\begin{key}{/tikz/animate/value=\meta{value}}
  ...
  Take care that when a value contains a comma, you must surround it
  by braces as in |"{(1,1)}"|.  
\end{key}
...
\begin{key}{/tikz/animate/with=|{|\meta{options}|}do{|\meta{forked options}|}|}
  This key first creates a local \TeX\ scope and first executes the
  \meta{options}, then opens another local \TeX\ scope, and then
  executes the \meta{forked options}. The only difference between the
...
\end{codeexample}
  ...
  It turns out the the |with| key can \emph{also} adds something to the
