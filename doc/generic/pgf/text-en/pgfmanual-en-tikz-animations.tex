...
\tikz{
  ...
  \begin{scope}[ animate me = { :drive = {
           ...
           duration = 36.5s, repeats }}] 
    \node {Earth};
    \node [animate me = { :drive = {
             along = {(0,0) circle [radius=1cm]},
             duration = 2.8s, repeats }}] {Moon};
  \end{scope}
}
\end{codeexample}
...
\item \emph{Before} or \emph{in the options of} the to-be-animated
  object you specify this object together with an \emph{attribute}
...
As can be seen, a special syntax in several places: Entries with a
colon such as |:xshift| specify an attribute, values
