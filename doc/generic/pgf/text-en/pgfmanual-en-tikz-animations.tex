...
\subsubsection{The Nature of Animations}
...
The options passed to the |animate| key
...
\begin{key}{/tikz/animate/object=\meta{list of objects}}
  ...
  to by \meta{object} will be the \emph{next} object with the |name|
...
\begin{key}{/tikz/animate/time=\meta{time}\opt{|later|}}
  Sets the time for the next time--value pair to \meta{time} (the text
  |later| is optional). 
...
\begin{key}{/tikz/animate/with=\marg{setup} do \marg{options}}
  This key first creates a local \TeX\ scope and first executes the
  \meta{setup}, then opens another local \TeX\ scope, and then
  executes the \meta{options}. The only difference between the
...
animate = { % two "with" statements, each creating an entry:
  with = {} do { object = obj, attribute = color, time = 0s, value = red },
  with = {} do { object = obj, attribute = color, time = 2s, value = blue }
}
\end{codeexample}
\begin{codeexample}[code only]
animate = { % two "with" statements, each creating an entry, but some keys set for both
...
\end{codeexample}
\begin{codeexample}[code only]
animate = { % three "with" statements, but only two create an entry
  with = {object = obj, attribute = color} do {
    with = {} do { time = 0s, value = red },
    with = {} do { time = 2s, value = red }
  } % outer with has no effect since, here, the time key is not specified
}
\end{codeexample}
\begin{codeexample}[code only]
animate = { % two "with" statements, each creating an entry, but no scoping at all 
  object = obj, attribute = color,
...
Inside an \meta{animation specification} (or, more generally, for all
...
\begin{quote}
  \normalfont
  |with = {object = |\meta{objects}|, attribute = |\meta{attributes}%
    |, id = |\meta{id}|} do {|\meta{options}|}|
\end{quote}
...
beginning of this scope the key |time=|\meta{time} is executed and
then the \meta{options} are executed inside this scope. Formally, the
following key is executed whenever the above syntax is encountered:
\begin{quote}
  \normalfont
  |with = {time = |\meta{time}|} do {|\meta{options}|}|
\end{quote}
...
mark\footnote{Of catcode 12 for those knowledgeable of such
  things.}. Then, a local scope is created in which the \meta{value}
is passed to the |value| key, described below, and, then, the
\meta{options} are executed in this scope. Note that when the
\meta{value} contains a comma, you have to surround it by curly braces
...
\begin{quote}
  \normalfont
  |with = {value = |\meta{value}|} do {|\meta{options}|}|
\end{quote}
...
\begin{key}{/tikz/animate/value=\meta{value}}
  Sets the value of the current time--value pair to \meta{value}. The
  syntax of the \meta{value} is not fixed, it depends on the type of
