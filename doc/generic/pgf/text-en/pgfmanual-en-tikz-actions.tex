...
When you draw a line, you can add arrow tips at the ends using the
following key:

\begin{key}{/tikz/tips=\meta{true or false}}
  When set, arrow tips will be added at the beginning and at the end
  of the current path (when the path consists of several subpaths,
  there rules are more complicated).

  This option is automatically selected by |draw|, so you normally do
  not give it explicitly. Its main purpose is to allow you to draw
  ``only'' arrow tips for a path without drawing the path itself. Here
  is an example:
...
Since specifying and configuring arrow tips is a bit of an art, there
is a whole section devoted to it, see Section~\ref{section-tikz-arrows}.
