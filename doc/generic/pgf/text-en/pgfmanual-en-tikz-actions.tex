...
\begin{key}{/tikz/cap=\meta{type} (initially butt)}
...
\begin{key}{/tikz/join=\meta{type} (initially miter)}
...
\begin{key}{/tikz/shorten >=\meta{dimension} (intially 0pt)}
...
\begin{tikzpicture}
  [
    ...
    button/.style={
      % First preaction: Background pattern
      preaction={pattern=#1,
                 path fading=circle with fuzzy edge 15 percent},
      % Second preaction: shiny effect
  ...
  \node [button] at (1,1) {Hello};
  \node [button=crosshatch dots light steel blue,
         text=white] at (2.2,1) {\tikzname};
\end{tikzpicture}
\end{codeexample}
\end{key}
