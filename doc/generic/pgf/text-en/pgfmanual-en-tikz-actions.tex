...
\begin{itemize}
  ...
  The only permissible \meta{name} is |dots| by default. To get more
  patterns, you need to load the file |pgflibrarypatterns|, see
  Section~\ref{section-library-patterns}. 
...
\begin{codeexample}[]
\begin{tikzpicture}[pattern=fivepointed stars]
  \draw[pattern color=red]  (0,0) circle (1cm);
  \draw[pattern color=blue] (0,0) rectangle (3,1);
\end{tikzpicture}
\end{codeexample}
\end{itemize}
