...
\begin{command}{\pgfpathquadraticcurveto\marg{support}\marg{coordinate}}
  This command works like |\pgfpathcurveto|, only it uses a quardatic
  Bézier curve rather than a cubic one. This means that only one
  suport point is needed. 
\begin{codeexample}[]
\begin{pgfpicture}
  \pgfpathmoveto{\pgfpointorigin}
...
\end{pgfpicture}
\end{codeexample}
  Internally, the quadartic curve is converted into a cubic
  curve. The only noticable effect of this is that the points used for
