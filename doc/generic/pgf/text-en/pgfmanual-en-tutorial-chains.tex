...
be shown! The trick is to us the |as| option, which allows you to
specify how a node show be rendered.
\begin{codeexample}[]
\tikz \graph [grow right sep] {
  unsigned integer[nonterminal] -> d[terminal, as=.] -> digit[terminal] -> E[terminal]
};  
\end{codeexample}
...
\tikz \graph [grow right sep] {
  ...
  {
    plus              [terminal, as=+],
    nothing           [coordinate, as=],
    minus             [terminal, as=-]
  } ->
  unsigned integer2 [nonterminal, as=unsigned integer]
};  
\end{codeexample}
Let us see, what is happening here. First note how Ilka had to write
|unsigned integer2| for the second unsigned integer. The reason is
...
|unsigned integer2| for a node that she will not reference again
anyway. In this case, she can use |_| as the name, which always stands
for a ``fresh, anonymous'' node name.
...
\begin{tikzpicture}[>=stealth', thick, black!50, text=black,
  ...
  \graph [grow right sep, branch down=7mm] {
    ...
    q3                [coordinate] --
    _                 [nonterminal, as=unsigned integer] --
    p6                [coordinate] ->
    _                 [coordinate];
...
\tikz [>=stealth', black!50, text=black, thick,
  ...
  \graph [grow right sep, branch down=7mm, simple] {
    _ -> unsigned integer[nonterminal] -- p1 -> _[terminal, as=.] -- p2 -> digit[terminal] --
    p3 -- p4 -- p5 -> E[terminal] -- q1 ->[vh path]
    {[nodes={yshift=7mm}]
      _[terminal, as=+], q2, _[terminal, as=-]
    } -> [hv path]
    q3 -- _[nonterminal, as=unsigned integer] -- p6 -> _;
