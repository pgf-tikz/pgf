...
\begin{tikzfadingfrompicture}[name=tikz]
  \node [text=transparent!0,above right]
...
\begin{tikzpicture}
  ...
  \path [pattern=checkerboard,pattern color=black!30] (-2,-1) rectangle (2,1);
  \begin{scope}[scope fading=tikz]
    \shade[left color=blue,right color=blue!50] (-2,-1) rectangle (2,1);
  \end{scope}
\end{tikzpicture}
\end{codeexample}
\end{environment}
...
\begin{command}{\tikzfadingfromshading\oarg{options}}
  This command is used to ``turn a shading into a fading.'' In the
  \meta{options} you should 
  \begin{enumerate}
  ...
  \item extra options for setting the colors of the shading (typically
    you will set them to |transparent!|\meta{percentage}).
  \end{enumerate}
  Then, a new fading named \meta{name} will be created based on the
  shading.

\begin{codeexample}[]
\tikzfadingfromshading[name=fade right,
...
\begin{tikzpicture}
  ...
  \fill [red,fading=fade right] (-1,-1) rectangle (1,1);
\end{tikzpicture}
\end{codeexample}  

\begin{codeexample}[]
\tikzfadingfromshading[name=fade out,
...
\begin{tikzpicture}
  ...
  \fill [blue,fading=fade out] (-1,-1) rectangle (1,1);
\end{tikzpicture}
\end{codeexample}  
\end{command}
...
fading for the current scope or path. The fading |fade down| used
in the following examples gets more and more transparent as we go from
top to bottom. 
...
\begin{tikzpicture}[fading=fade down]
  ...
  \fill [red,fade,fading transform={rotate=90}]  (1,0.75) ellipse (.75 and .5);
  \fill [red,fade,fading transform={rotate=30}] (3,0.75) ellipse (.75 and .5);
\end{tikzpicture}
\end{codeexample}
  \end{key}
...
\begin{tikzpicture}
  ...
  \shade [ball color=white,fading=fade inside] (2,2) circle (1.8);
\end{tikzpicture}
\end{codeexample}

  Note that adding the |fade| option to a node fades the (background)
  path, not the text itself. To fade the text, you need to use a scope
  fading (see below).
\end{key}
...
scope. This can be achieved using the |scope fading| option, often in
conjunction with a |transparency group|, described at the end of this
section.

\begin{key}{/tikz/scope fading=\meta{fading}}
  This key only has an effect when used with a scope. In this case, it
  will install the \meta{fading} as the current fading for the whole
  scope. This means that all drawing done inside the scope is subject
  to fading. It is not obvious which size the scope fading should
  have. There are two possibilities.

  First, the scope fading can have its ``natural size.'' This size is
  set when the fading is created. Many fadings have a size of 100bp by
  100bp, which is not very useful, but more special-purpose fadings
  may have other sizes. In this case, the fading is centered at the
  origin. Since this position is rarely desirable, you should use the
  |fading transform| option, which also applies to scope fadings, to
  shift the fading where you need it. 

  Second, the scope fading can be automatically resized an shifted to
  cover a certain path. This path is specified using the
  |scope fading bounding box| option and it is only used for the
  computation of the size of the fading; it does serve any other
  purpose.

  The first method is used when the path stored in
  |scope fading bounding box| is empty, otherwise the second method is
  used. 

  \begin{key}{/tikz/scope fading bounding box=\meta{path}}
    When a scope fading is installed, the \meta{path} will be
    processed and a bounding box is computed for it. Then the scope
    fading's size is adjusted to cover this path in the usual
    fashion. The path is not used otherwise and discarded.
  \end{key}

  In the first example, the fading is used at its natural size. For
  clarity, the bounding box of the fading is also shown.
  
\begin{codeexample}[]
\begin{tikzpicture}
  \fill [black!20] (-2,-2) rectangle (2,2);
  \path [pattern=checkerboard,pattern color=black!30] (-2,-2) rectangle (2,2);

  % The bounding box of the shading:
  \draw [red] (-50bp,-50bp) rectangle (50bp,50bp);
  
  \begin{scope}[scope fading=fade down]
...
\begin{tikzpicture}
  ...
  \begin{scope}[scope fading=fade down,
                scope fading bounding box={(-2,-2) rectangle (2,2)}]
    \fill[red]   ( 90:1) circle (1);
    \fill[green] (210:1) circle (1);
    \fill[blue]  (330:1) circle (1);
  \end{scope}
...
\end{codeexample}
  ...
  Scope fadings are also needed if you wish to fade text.
\begin{codeexample}[]
\begin{tikzpicture}
  [scope fading=fade down,fading angle=45,
   scope fading bounding box={(0,0) rectangle (4,2)}]
  \node [below right,text width=4cm,inner sep=0pt] at (0,2)
  {
