...
library for the following example):
\begin{codeexample}[]
\tikz {
  \node [circle,draw] (A)              {A};
...
deprecated. Use |arrows.meta| instead, which allows you to do all that
the old libraries offered, plus much more.
...
\begin{key}{/pgf/arrow key/width'=\meta{dimension}| |\opt{\meta{length
...
\begin{key}{/pgf/arrow key/inset'=\meta{dimension}| |\opt{\meta{length factor}}| |\opt{\meta{line width factor}}}
...
\begin{key}{/pgf/arrow keys/arc=\meta{degrees} (initially 180)}
  Sets the ``length'' of the arc to \meta{degrees}. Note that this key
  is quite different from the \meta{angle} key, which is ``just a
...
\begin{key}{/pgf/arrow key/line width'=\meta{dimension}| |\opt{\meta{length factor}}}
...
\item \emph{Barbed} arrow tips consist mainly of lines that ``point
  ...
  \draw (0,.5ex) -- (1.5em,.5ex) [-Straight Barb];.
...
In addition to the above, the library makes two special definitions:

\begin{enumerate}
\item The standard |>| arrow tip is (re)defined as |To[sep]|.
\item The standard |To| arrow tip is (re)defined as
