...
\begin{itemize}
  ...
  This option has the following effects: First, |anchor=south| is
  set. Second, the anchor \meta{node}|.90| is computed, which is the
  northern border of the node \meta{node}. Third, the current node is
  placed in such a way that its |south| anchor lies on the computed
  position plus a $y$-offset of |node distance|.
\begin{codeexample}[]
...
\end{codeexample}
  ...
  options. Note that this distance is not the distance between the
  centers of the nodes, but the distance between their borders.
\end{itemize}
...
\end{codeexample}
  ...
  \meta{direction} of |false| disabled automatic placement. This
...
\begin{pathoperation}{edge}{\opt{|[|\meta{options}|]|}
  ...
  If there are several |to| operations in a row, the start coordinate
  ...
  coordinate preceding the first |to| operation. This is
