...
\begin{key}{/tikz/label=\opt{|[|\meta{options}|]|}\meta{angle}|:|\meta{text}}
  ...
  \meta{text}. It is placed according to the following rules: 
  \begin{enumerate}
  \item Suppose the |node| currently under construction is called
    |main node| and let us call the label node |label node|.
  \item If the |transform shape| option is not set for the
    |label node| (neither locally inside \meta{options} nor inherited), then
    the coordinate transformation system is completely reset.
  \item Then, an anchor point for the |label node| is computed that
    lies on the border of the |main node|, namely in the direction
    \meta{angle} from the center of the node. Note that this typically
    the same as |main node.|\meta{angle}, but not if the |main node|
    has been transformed itself using the |transform shift| option.
  \item The anchor that is chosen depends on the \meta{angle}. If the
    \meta{angle} lies between $-3^\circ$ and $+3^\circ$, then the
    anchor |west| is chosen, which causes |label node| to be placed
    right of the right end 
...
\end{codeexample}
  ...
  Here are examples that show the effects of different settings of
  the |transform shape| options.
\begin{codeexample}[]
\tikz [rotate=60]
  \node [rectangle,draw,label=right:label] {main node};
\end{codeexample}
\begin{codeexample}[]
\tikz [rotate=60]
...
\begin{codeexample}[]
\tikz [rotate=60]
  \node [rectangle,draw,label={[transform shape]right:label}] {main node};
\end{codeexample}
\begin{codeexample}[]
\tikz [rotate=60]
  \node [transform shape,rectangle,draw,
         label={[transform shape]right:label}] {main node};
\end{codeexample}
