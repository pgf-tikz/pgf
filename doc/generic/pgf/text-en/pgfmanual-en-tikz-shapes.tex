...
\subsubsection{Text Nodes for the Tex4ht driver}

Text Nodes aren't supported well with the tex4ht driver  
...
When you write |node[tex4ht node/escape=false] {text}|, the |text| is translated into svg text, which is somewhat limited :  
simple characters (letters,numerals,ponctuation,$\sum$,$\int$,\ldots), subscripts and supscripts (but not subsupscripts) will display but everything else will be filtered out, ignored or will produce invalid html code (in the worst case). This means that two kind of texts render reasonably well: 
	\begin{enumerate}
	\item First, plain text without math mode, special
    characters or anything else special. 
    \item Second, \emph{very} simple
    ...
    simple as |$\alpha$|, this may corrupt the graphic. So, once more,
    \emph{stick to very simple node text with this rendering method.}
    
\begin{codeexample}[code only]
\tikz \node[draw,tex4ht node/escape=false] {Exemple : $(a+b)^2=a^2+2ab+b^2$};          
\end{codeexample}

    When you write |node[tex4ht node/escape=true] {text}|, tikz escapes back to html to render the |text|. There are a few advantages and a few caveats to this rendering method :

      \begin{enumerate}
    \item Advantages
    \subitem This method produces valid html codes in most cases actually. 
    \subitem The support for complicated text nodes is much better : code that renders well outside a tikzpicture, should render well inside a text node.
    \subitem Inside text nodes with fixed width, html will produce line breaks for long lines.
    \item Caveats
    \subitem you need a browser with good svg support (Firefox 3+,Google chrome,Safari,Internet Explorer with the Adobe Svg Plugin,...) to display the picture
    \subitem The text will display differently, depending on your browsers, the fonts you have on your system and your settings.
    \subitem tikz has to guess the size of the text rendered by the browser to scale it and prevent it from sticking from the node. Sometimes, it will fail and the text will be either cropped or too small.
  \end{enumerate}
    \begin{codeexample}[code only]
    \tikz \node[draw,tex4ht node/escape=true] {Exemple : $\int_0^\infty\frac{1}{1+t^2}dt=\frac{\pi}{2}$};          
\end{codeexample}
\end{key}
...
\begin{key}{/tikz/tex4ht node/id=\meta{id name} (default |\string\jobname\ picture number-node number|)}
