...
\begin{key}{/tikz/above=\opt{\meta{specification}} (default 0pt)}
  With the |placements| library loaded, the |above| option does not
...
\end{tikzpicture}
\end{codeexample}
    This use of the |above| option is the same as if the |placements|
...
The |placements| package also introduces the following new placement
keys:
...
are placed ``in a row.'' An alternative, more powerful way is to use
matrices, see Section~\ref{section-matrices}, but for simple cases
chains will be easier to use. You need to load the |placements|
library to use chains.

The general idea behind a chain is the following: A chain is a
sequence of nodes where each node is placed relative to the
preceding one. Typically, you construct one chain at a time, but it is
...
The first step toward creating a chain is to use the |setup chain|
key.
\begin{key}{/tikz/setup chain=\meta{chain name}|going |\meta{direction}}
  This key should be given as an option to a scope enclosing all nodes
  of the chain. Typically, this will be a |scope| or the whole
  |tikzpicture|, but it might just be a path on which all nodes are
  placed.

  The key installs a chain named \meta{chain name} for the current
  scope. Inside this scope, you cannot use |setup chain| once more for
  the same \meta{chain name}, you have to use a different one. It is
  permissible that the \meta{chain name} is empty; in this case the
  chain name is simply the empty string.

  The \meta{direction} is used to determine the placement rule for
  nodes on the chain. The placement rule is just a \tikzname\ option
  that is applied automatically to each node on the chain. Depending
  on the form of the \meta{direction} parameter, the placement rule is
  determined in two different ways:
  \begin{enumerate}
  \item If \meta{direction} contains an equal sign, then
    \meta{direction} is used as the placement rule. 
  \item If \meta{direction} does not contain an equal sign, then
    \meta{direction}|=of \tikzchainprevious| is used as the placement
    rule. 
  \end{enumerate}
...
\begin{codeexample}[]
\begin{tikzpicture}[setup chain=1 going right,
                    setup chain=2 going below,
                    node distance=5mm,
...
\end{codeexample}
\begin{codeexample}[]
\begin{tikzpicture}[setup chain=going right]
  % Use the empty chain    
  \node [on chain] {A};
  \node [on chain] {B};
  \node [on chain] {C};
...
\begin{key}{/tikz/on chain=\opt{\meta{chain name}}\opt{|going |\meta{new direction}}}
  ...
  the current scope using the |setup chain|. If \meta{chain name} is
  the empty string, the chain whose name is the empty chain must have
  been declared.

  The |going |\meta{new direction} part is optional. If present it
  changes the placement rule for the present and all subsequent nodes
  in the same way as described for |setup chain|.
  ...
  \item If the node does not yet have a name, (having been given using
    ...
    the empty chain, the first node is named |-1|, the second |-2|,
    and so on.
  ...
  \item Except possibly for the first node of the chain, the placement rule is
    now executed. Inside this rule, the macros |\tikzchainprevious| is
    typically used to access the previous node, but you can also
    access |\tikzchaincurrent| or |\tikzchaincount| for more
    complicated placings. The follow key governs whether the placement
    rule is also executed for the first node:
    \begin{key}{/tikz/chain is relative=\meta{boolean} (initially true)}
      If set to |true|, each node is placed relative to the previous
      one, which means that the placement rule is not executed for the
      first node of a chain. If set to |false|, the placement rule is
      also applied to the first node of a chain. Note that for this
      first node the  |\tikzchainprevious| is empty. Here is an
      example where this   option makes sense:
\begin{codeexample}[]
\begin{tikzpicture}[setup chain=circle going {at=(\tikzchaincount*30:1.5)},
                    chain is relative=false]
  \foreach \i in {1,...,10}
    \node [on chain=circle] {\i};
...
\begin{codeexample}[]
\begin{tikzpicture}[setup chain=going right,node distance=5mm]
  \node [draw,on chain] {};
...
\end{codeexample}
  ...
  The optional \meta{new direction} allows us to change the direction
...
\begin{tikzpicture}[setup chain=going right,node distance=5mm]
  ...
  \node [draw,on chain] {it.};
\end{tikzpicture}
\end{codeexample}

  You can also use more complicated computations in the \meta{direction}:
\begin{codeexample}[]
\begin{tikzpicture}[setup chain=going {at=(\tikzchainprevious),shift=(30:1)}]
  \draw [help lines] (0,0) grid (3,2);
  \node [draw,on chain] {1};
  \node [draw,on chain] {Hello};
  \node [draw,on chain] {World};
...
\begin{key}{/tikz/chain edge=\meta{options}}
  When this key is given to any node on the chain, except for the
  first (where this option has no effect), an |edge| command is added
  after the node. This |edge| command gets the \meta{options} as a
  parameter and gets |\tikzchainprevious| as its target. The net
  effect of this is that the current chain node is connected to the
  previous chain node via an edge.
\begin{codeexample}[]
\begin{tikzpicture}[setup chain=going right,node distance=5mm,
                    every on chain/.style={chain edge={red,<-}},
...
\begin{tikzpicture}[
  ...
  \begin{scope}[setup chain=going right,
                every node/.style={on chain},
                terminal/.append style={chain edge={<-,shorten <=1pt}},
                nonterminal/.append style={chain edge={<-,shorten <=1pt}},
                support/.style={chain edge={-}}]
    \coordinate [support]     (start);
    \node       [nonterminal] {unsigned integer};
    \coordinate [support]     (after ui);
    \node       [terminal]    {.};
    \coordinate [support]     (after dot);
    \node       [terminal]    {digit};
    \coordinate [support]     (after digit);
    \coordinate [support]     (skip);
    \coordinate [support]     (before E);
    \node       [terminal]    {E};
    \coordinate [support]     (after E);
    \coordinate [support,
                 xshift=5mm]  (between);
    \coordinate [support,
                 xshift=5mm]  (before last);
    \node       [nonterminal] {unsigned integer};
    \coordinate [support]     (after last);
    \coordinate [chain edge={<-}] (end);
  \end{scope}
