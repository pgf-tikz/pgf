...
\end{codeexample}
\end{key}
\begin{key}{/tikz/above left=\meta{offset} (default 0pt)}
...
\begin{key}{/tikz/above right=\meta{offset} (default 0pt)}
...
\end{key}
\begin{key}{/tikz/below left=\meta{offset} (default 0pt)}
  Does the same as |anchor=north east|.
\end{key}
\begin{key}{/tikz/below right=\meta{offset} (default 0pt)}
...
\tikz [pin distance=15mm,
       every pin edge/.style={<-,shorten <=1pt,snake=snake,
                              line before snake=4pt}]
  \node [circle,draw,pin=right:X,
                     pin=above right:Y,
                     pin=above:Z]       {my circle};
\end{codeexample}
...
\end{codeexample}
  ...
  \begin{stylekey}{/tikz/every edge (intitially draw)}
...
\begin{key}{/tikz/remember picture=\meta{boolean} (default true, initially false)}
