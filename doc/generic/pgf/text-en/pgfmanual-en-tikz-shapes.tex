...
key.
\begin{key}{/tikz/setup chain=\opt{\meta{chain name}}|going |\meta{direction}}
  ...
  the same \meta{chain name}, you have to use a different one. If no
  \meta{chain name} is given, |chain| will be used instead.
  ...
  \begin{key}{/tikz/current chain=\meta{name}}
    This key (locally) keeps track of the current chain that is being
    constructed. This name will be used by the |on chain| option if no
    chain name is given.

    So: If you do not provide a \meta{chain name} for the
    |setup chain| command, |chain| is used, while if you do not
    provide a \meta{chain name} for the |on chain| command, the value
    of |current chain| is used.
  \end{key}
...
\begin{key}{/tikz/on chain=\opt{\meta{chain name}}\opt{|going |\meta{new direction}}}
  ...
  the empty string, the current value of |current chain| is used. 
...
\begin{tikzpicture}[
  ...
  \begin{scope}[setup chain=going right,
                ...
                support/.style={coordinate,chain edge={-}}]
    \node [support]             (start)        {};
