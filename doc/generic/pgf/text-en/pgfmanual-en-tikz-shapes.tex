...
chains will be easier to use. You need to load the |positioning|
...
The first step toward creating a chain is to use the |chain|
key.
\begin{key}{/tikz/chain=\opt{\meta{chain name}}\opt{\meta{direction}}}
  ...
  scope. Inside this scope, you cannot use |chain| once more for
  ...
  \begin{key}{/tikz/current chain=\meta{name}}
    ...
    |chain| option, |chain| is used as the name, while if you do not
...
\begin{codeexample}[]
\begin{tikzpicture}[chain=1 going right,
                    chain=2 going below,
                    node distance=5mm,
...
\begin{key}{/tikz/on chain=\opt{\meta{chain name}}\opt{\meta{direction}}}
  ...
  the current scope using the |chain| option. If \meta{chain name} is
  the empty string, the current value of the key |current chain| is used. 
  ...
  that was given to the original |chain| option is used.
...
\begin{codeexample}[]
\begin{tikzpicture}[chain=circle placed {at=(\tikzchaincount*30:1.5)}]
  \foreach \i in {1,...,10}
    \node [on chain] {\i};
...
\begin{codeexample}[]
\begin{tikzpicture}[chain=going right,node distance=5mm]
  \node [draw,on chain] {};
...
\begin{codeexample}[]
\begin{tikzpicture}[chain=going {at=(\tikzchainprevious),shift=(30:1)}]
  \draw [help lines] (0,0) grid (3,2);
  \node [draw,on chain] {1};
  \node [draw,on chain] {Hello};
  \node [draw,on chain] {World};
...
\begin{codeexample}[]
\begin{tikzpicture}[chain,node distance=5mm,
...
\subsubsection{Arranging Nodes Using a Matrix}

You may often wish to arrange nodes in a table or matrix such that
there are columns and rows of nodes. \tikzname\ provides its own,
quite general, |matrix| option for such cases. Since the use of this
option is a bit complex, it is described in a separate
