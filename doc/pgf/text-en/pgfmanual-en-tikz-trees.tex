...
node on a path. To do so, you can use the |child| and the |children|
operation:
...
\begin{pathoperation}{children}{\opt{\oarg{options}%
      \meta{variables}|in|}\marg{values}\marg{child path}}%
  This path operation can be used to add multiple children to a
  node. The idea is the following: A |\foreach| statement is used to
  iterate over the list of \meta{values}. For each value in this list,
  a new |child| is added to the node. The syntax for \meta{variables}
  and for \meta{values} is the same as for the |\foreach| statement,
  see Section~\ref{section-foreach}. The \meta{options} are given as
  options to each produced |child|.

  For example, when you say
\begin{codeexample}[code only]
node {root} children[red] \name in {1,2} {node {\name}}
\end{codeexample}
...
\end{codeexample}
  ...
  You can leave out the \meta{variables}|in|. In this case, a dummy
  variable is automatically used. This allows you to write things like
  the following code:
\begin{codeexample}[]
\tikz \node{parent} children {1,...,3} {node {child}}; 
\end{codeexample}

  Naturally, the more interesting case occurs when you nest things:
\begin{codeexample}[]
\begin{tikzpicture}[level distance=4mm]
  ...
  \coordinate children {0,1} {children {0,1} {children {0,1} {}}};
\end{tikzpicture}
\end{codeexample}
\end{pathoperation}
