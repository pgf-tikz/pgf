...
the dashing pattern. The options that govern how paths are drawn all
start with |\pgfset|. \emph{All options that influence how a path is
  rendered always influence the complete path.} Thus, it is not
possible to draw part of a path using, say, a red color and drawing
another part using a green color. To achieve such an effect, you must
use two paths.
...
parameters influence thow the path is drawn. The commands for setting
...
persists past \TeX\ groups, \emph{except} for the iterior rule
(even-odd or non-zero) and the arrow type. The latter to graphic
parameter only persist till the end of the current \TeX\ group, but
...
\begin{command}{\pgfsetlinewidth\marg{line width}}
  This command sets the line width for subsequent strokings (in the
...
\begin{command}{\pgfsetstrokecolor\marg{color}}
  ...
  the |\color| command, the effect of this command last till the end
  ...
  provide \LaTeX\ colors. For this reasong, \pgfname\ implements a
  minimalistic ``emulation'' of the |\defincolor|, |\colorlet|, and
  |\color| commands. Only grayscale and rgb colors are supported. For
...
After a path has been drawn, \pgfname\ can add arrows at the
ends. Currently, it will only add arrows correctly at the end of paths
the consist of a single open part. For other paths, like closed paths
or path consisting of multiple parts, the result is not defined.

\begin{command}{\pgfsetarrowsstart\marg{arrow kind}}
  Sets the arrow king used at the start of a (possibly curved)
...
\begin{command}{\pgfsetarrowsend\marg{arrow kind}}
  Sets the arrow king used at the end of a path.
\begin{codeexample}[]
\begin{pgfpicture}
  \pgfsetarrowsstart{latex}
  \pgfsetarrowsend{to}
...
\begin{command}{\pgfsetshortenstart\marg{dimension}}
  This command will shortend the start of every stroked path by the
  ...
  This command is useful if you wish arrows or line to ``stop shortly
...
the current fill color. It is not always obivous whether a point is
...
the even-odd rule or the non-zero rule.
...
\subsection{Using a Path as Bounding Box}
\label{section-using-bb}

When you add the |use as bounding box| option, the bouding box of the
...
effect in the size of the bounding box. Typically, you use this
