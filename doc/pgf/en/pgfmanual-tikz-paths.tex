...
call it $y$. The curve is a cubic Bézier curve. For such a curve,
...
The syntax of the parabola operation is
\declare{|parabola|\opt{\oarg{options}|bend|\meta{bend
      coordinate}}\meta{coordinate}}. This will add a parabola through
the current point and the given \meta{coordinate}. If the |bend| is
given, it specifies where the bend should go; the \meta{options}
can also be used to specify where the bend is. By default, the bend
is at the old current point. 
...
\begin{itemize}
  ...
  Has the same effect as saying |bend|\meta{coordiante} outside the
