...
\begin{command}{\pgfsys@beginpicture\marg{$x_1$}\marg{$y_1$}\marg{$x_2$}\marg{$y_2$}}
  Called at the beginning of a |{pgfpicture}|. The parameters will be
  set to the bounding box of the picture, where $(x_1,y_1)$ is the
  lower left corner and $(x_2,y_2)$ is the upper right corner. All
  values are given as \TeX\ dimensions. In order to have these sizes
  available when this command is called, the picture is actually
  typeset inside a box \emph{before} this command is called. Then,
  once the size has been calculated, this command is called with the
  size of the picture and the box is inserted.

  Note that the bounding box is just ``for your information.'' The
  |{pgfpicture}| environment will take care of reserving the right
  amount of space for the box; it is not necessary that this command
  reserves any space. For this reason, most drivers can ignore the
  parameters; currently only the \textsc{svg} driver needs this
  information.  
...
\begin{command}{\pgfsys@beginhbox}
  Called before a (horizontal) TeX box is typeset inside a
  |{pgfpicture}|. By default, this just opens a scope.
...
\begin{command}{\pgfsys@endhbox}
  Called after a TeX box has been typeset inside a |{pgfpicture}|. By
