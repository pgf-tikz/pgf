...
\begin{tikzpicture}[Venn diagram=
	{labels={"X","Y","Z"}}]
 \Venn{XuY={fill=blue,opacity=0.5},
   YnZ={fill=red,opacity=0.5}}
\end{tikzpicture}
\end{codeexample}
...
It may be important to know that one can use |Venn diagram| in scopes.
...
\begin{key}{/tikz/reverse clip }
		...
		the clip paths matters. Therefore there is |/tikz/reverse clip'|
...
\begin{tikzpicture}
 ...
 \begin{scope}[Venn diagram,local bounding box=by hand]
   \clip (cC) circle[radius=\pgfkeysvalueof{/tikz/venn/radius}] [reverse clip];
   \path[fill=blue,venn/and={A and B}];
 \end{scope}
...
\begin{tikzpicture}[Venn diagram,
 declare function={
 R=\pgfkeysvalueof{/tikz/venn/radius};}]
 \clip[even odd clip] 
  (cB) circle[radius=R] 
  (cC) circle[radius=R];
 \fill[orange] (cA) circle[radius=R];
\end{tikzpicture}
\end{codeexample}
