...
% \pgf@sys@bp@correct
%
% According to Chapter 10: Dimensions of the TeXbook
%
%   1 in = 72.27 pt
%   1 in = 72 bp
%
% thus to scale bp to pt we would need to calculate
%
%   pt = 72 / 72.27 * bp ≈ 0.9962640099626401 * bp
%
% which is then rounded to 0.99627 but that incurs a rounding error. Instead
% we have to realise that it is better to perform the calculation in integer
% arithmetic for which we use the fact
%
%   pt = 72 / 72.27 * bp = 7200 / 7227 * bp = 800 / 803 * bp = (2 * bp) * 400 / 803
%
% which is implemented below additionally compensating (2 * bp) by 1sp depending
% on the sign for correct rounding, see \dim_to_decimal_aux:w in l3skip for
% details.
\def\pgf@sys@bp@correct#1{#1=\expandafter\pgf@sys@bp@correct@\number#1;400/803;}
\def\pgf@sys@bp@correct@#1#2;#3;{\dimexpr\the\numexpr(2*#1#2\if#1--\else+\fi 1)*#3\relax sp\relax}
