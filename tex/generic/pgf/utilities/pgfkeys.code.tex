...
% same \pgfkeysvalueof but the value is wrapped in \unexpanded
\long\def\pgfkeysunexpandedvalueof#1{%
  \pgfkeys@unexpanded\expandafter\expandafter\expandafter{%
    \csname\pgfkeys@ifcsname{pgfk@#1}{pgfk@#1}{pgfkeys@relax}\endcsname}}

% same \pgfkeysvalueof but the value is wrapped in \detokenize
\long\def\pgfkeysdetokenizedvalueof#1{%
...
% If for testing whether a key is empty
%
% #1 = key
% #2 = if-case
% #3 = else-case
...
% Description:
%
% This if will be executed if the key is empty.  A non-existent key expands to
% \pgfkeys@relax (a synonym for \relax) and is therefore not empty!
%
% Example:
%
% \pgfkeysifempty{/tikz/length}{key is empty}{key is not empty (or does not exist)}

\long\def\pgfkeysifempty#1{%
  \if\relax\pgfkeysdetokenizedvalueof{#1}\relax
    \expandafter\pgfkeys@firstoftwo
  \else
    \expandafter\pgfkeys@secondoftwo
  \fi
