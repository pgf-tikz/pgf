...
\def\pgfpointintersectionoflines#1#2#3#4{%
  {%
    %
    % Process arguments to avoid variable evaluation errors
    %
    \pgfextract@process\arg@one{#1}
    \pgfextract@process\arg@two{#2}
    \pgfextract@process\arg@three{#3}
    \pgfextract@process\arg@four{#4}
    %
    ...
    %
    \pgf@process{\arg@four}%
    \pgf@xb=\pgf@x%
    \pgf@yb=\pgf@y%
    \pgf@process{\arg@three}%
