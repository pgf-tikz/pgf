...
% hypot function.
%
\pgfmathdeclarefunction{hypot}{2}{%
    \begingroup
        % x = abs(x)
        \pgfmathabs@{#1}%
        ...
        % y = abs(y)
        \pgfmathabs@{#2}%
        \pgfmath@y\pgfmathresult pt
        % x = max(x,y), y = min(x,y)
        \ifdim\pgfmath@x<\pgfmath@y
            \pgfmath@xa\pgfmath@x\relax
            \pgfmath@x\pgfmath@y\relax
            \pgfmath@y\pgfmath@xa\relax
        \fi
        % if one operand is zero the result is the other operand
        \ifdim\pgfmath@x=0pt
            \edef\pgfmathresult{\pgfmath@tonumber{\pgfmath@y}}%
        \else
            \ifdim\pgfmath@y=0pt
                \edef\pgfmathresult{\pgfmath@tonumber{\pgfmath@x}}%
            \else
                % y / x (expand argument list to not clobber @x and @y registers inside divide)
                \edef\pgfmath@temp{{\pgfmath@tonumber{\pgfmath@y}}{\pgfmath@tonumber{\pgfmath@x}}}%
                \expandafter\pgfmathdivide\pgfmath@temp
                % (y / x)**2
                ...
                % x * sqrt(1 + (y / x)**2)
                \pgfmathmultiply@{\pgfmath@tonumber{\pgfmath@x}}{\pgfmathresult}%
            \fi
        \fi
        \pgfmath@smuggleone\pgfmathresult
    \endgroup
