    ...
    \begingroup%
        ...
        \else%
            ...
            \ifdim0pt>\pgfmath@y%
                ...
                \pgfmath@y=-\pgfmath@y%
            \fi%
            % We scale both arguments x and y up by a factor of 100000, such that
            %
            %   x and y = #1#2#3#4#5#6 = #1.#2#3#4#5#6 * 100000
            %
            % which is safe because TeX prints at most five decimal digits and #7 will only
            % contain a (potentially unknown) number of excess zeros.  Then we can compute
            % x * 65536 / y using \numexpr which uses a 64-bit intermediate when a
            % multiplication is followed by a division, so even if the intermediate x *
            % 65536 exceeds \maxdimen or the maximum \count, it will still be preserved
            % correctly.  We also don't have to worry about the scaling factor, because
            %
            %   x * 65536 / y = (x * 100000) * 65536 / (y * 100000)
            %
            % The upscaling by 100000 has another reason, namely that for extremely small
            % values we would not improve anything if we didn't upscale, because having a
            % 64-bit intermediate doesn't magically give our input more bits of precision.
            \edef\pgfmath@tempa{\pgfmath@tonumber{\pgfmath@x}}%
            \edef\pgfmath@tempb{\pgfmath@tonumber{\pgfmath@y}}%
            \edef\pgfmath@tempa{\expandafter\pgfmathdivide@scale\pgfmath@tempa0000000\pgfmath@}%
            \edef\pgfmath@tempb{\expandafter\pgfmathdivide@scale\pgfmath@tempb0000000\pgfmath@}%
            % Even though we perform the division in integer arithmetic, we need
            % to assign the result back to a \dimen and therefore have to make
            % sure that we don't overflow it.  For that we check whether the
            % integer division of x / y exceeds the rounded integer value
            % \maxdimen and if it doesn't we perform the division in sp and
            % convert back to a \dimen.
            \ifnum\numexpr\pgfmath@tempa/\pgfmath@tempb\relax<16384
                \pgfmath@x=\pgfmath@sign\dimexpr\the\numexpr\pgfmath@tempa*65536/\pgfmath@tempb\relax sp\relax
            \else%
                \pgfmath@error{You've asked me to divide `#1' by `#2', %
                    but the result is too large}{}%
                \pgfmath@x=\pgfmath@sign\maxdimen\relax
            \fi%
        \fi%
        \pgfmath@returnone\pgfmath@x%
    \endgroup%
}
\def\pgfmathdivide@scale#1.#2#3#4#5#6#7\pgfmath@{#1#2#3#4#5#6}
...
\def\pgfmathreciprocal@@#1.#2#3#4#5#6#7\pgfmath@{%
        % We scale the argument up by a factor of 100000, such that
        %
        %   M = #1#2#3#4#5#6 = #1.#2#3#4#5#6 * 100000
        %
        % which is safe because TeX prints at most five decimal digits and #7
        % will only contain a (potentially unknown) number of excess zeros.  Then we can
        % compute 65536 * 100000 / M using \numexpr which uses a 64-bit intermediate when
        % a multiplication is followed by a division, so even if the intermediate 65536 *
        % 100000 exceeds \maxdimen or the maximum \count, it will still be preserved
        % correctly.
        %
        % The upscaling by 100000 has another reason, namely that for extremely small
        % values we would not improve anything if we didn't upscale, because having a
        % 64-bit intermediate doesn't magically give our input more bits of precision.
        \pgfmath@x=\dimexpr\the\numexpr65536*100000/#1#2#3#4#5#6\relax sp\relax%
        \pgfmath@returnone\pgfmath@x%
    \endgroup%
}
