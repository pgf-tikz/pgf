% Copyright 2019 by Jonathan P. Spratte
%
% This file may be distributed and/or modified
%
% 1. under the LaTeX Project Public License and/or
% 2. under the GNU Public License.
%
% See the file doc/generic/pgf/licenses/LICENSE for more details.

\ProvidesFileRCS{pgfmoduleparserx.code.tex}

%
% This file defines commands for defining a letter-by-letter parser
% based on a dfa. The idea is inspired by the pgf module parser, but extends it.
%

% options
\long\def\pgfparserxset#1%
  {%
    \pgfset{/pgfparserx/.cd,#1}%
  }
\pgfparserxset
  {%
    ,silent/.is if=\pgfparserx@ifname{every parser silent}%
    ,status/.is if=\pgfparserx@ifname{pgfparserx status}%
  }

% logic helpers
\long\def\pgfparserx@fi@Bn\fi#1{\fi}
\long\def\pgfparserx@fi@Bg\fi#1#2{\fi}
\long\def\pgfparserx@fi@BTb\fi#1#2#3{\fi#2}
\long\def\pgfparserx@fi@BTc\fi#1#2#3{\fi#3}

% opt arg parsing
\long\def\pgfparserxifmark#1%
  {%
    \ifx\pgfparserx@mark#1%
      \pgfparserx@fi@BTb
    \fi
    \pgfutil@secondoftwo
  }
\pgfutil@protected\long\def\pgfparserx@oarg@oarg#1%
  {%
    \pgfutil@ifnextchar[{\pgfparserx@oarg@oarg@{#1}}{#1{\pgfparserx@mark}}%
  }
\long\def\pgfparserx@oarg@oarg@#1[#2]%
  {%
    #1{#2}%
  }
\pgfutil@protected\long\def\pgfparserx@oarg@Oarg#1#2%
  {%
    \pgfutil@ifnextchar[{\pgfparserx@oarg@oarg@{#2}}{#2{#1}}%
  }
\pgfutil@protected\long\def\pgfparserx@oarg@darg#1#2#3%
  {%
    \long\def\pgfparserx@oarg@darg@##1#1##2#2{##1{##2}}%
    \pgfutil@ifnextchar#1{\pgfparserx@oarg@darg@{#3}}{#3{\pgfparserx@mark}}%
  }
\pgfutil@protected\long\def\pgfparserx@oarg@Darg#1#2#3#4%
  {%
    \long\def\pgfparserx@oarg@darg@##1#1##2#2{##1{##2}}%
    \pgfutil@ifnextchar#1{\pgfparserx@oarg@darg@{#4}}{#4{#3}}%
  }
\pgfutil@protected\long\def\pgfparserx@oarg@targ@#1%
  {%
    \def\pgfparserx@oarg@targ@next{#1}%
    \afterassignment\pgfparserx@oarg@targ@next
    \let\pgfparserx@ignored@token=%
  }
\pgfutil@protected\long\def\pgfparserx@oarg@targ#1#2%
  {%
    \pgfutil@ifnextchar#1%
      {\pgfparserx@oarg@targ@{#2{\pgfparserx@mark}}}{#2{\pgfparserx@stop}}%
  }

% status messages
\pgfutil@protected\def\pgfparserx@msg#1%
  {%
    \immediate\write0{(pgfparserx) #1}%
  }
\pgfutil@protected\def\pgfparserx@status@start#1%
  {%
    \pgfparserx@if{pgfparserx status}{\pgfparserx@msg{parser '#1' started}}{}%
  }
\pgfutil@protected\def\pgfparserx@status@state#1%
  {%
    \pgfparserx@if{pgfparserx status}
      {\pgfparserx@msg{|= switched to state '#1'}}
      {}%
  }
\pgfutil@protected\def\pgfparserx@status@rule
  {%
    \pgfparserx@if{pgfparserx status}
      {\pgfparserx@msg{| \space|= rule '\meaning\pgfparserxtoken' executed}}
      {}%
  }
\pgfutil@protected\def\pgfparserx@status@rule@all
  {%
    \pgfparserx@if{pgfparserx status}
      {%
        \pgfparserx@msg
          {| \space|= (all) rule '\meaning\pgfparserxtoken' executed}%
      }
      {}%
  }
\pgfutil@protected\def\pgfparserx@status@rule@undefined
  {%
    \pgfparserx@if{pgfparserx status}
      {%
        \pgfparserx@msg
          {| \space|= unknown for '\meaning\pgfparserxtoken' executed}%
      }
      {}%
  }
\pgfutil@protected\def\pgfparserx@status@rule@undefined@all
  {%
    \pgfparserx@if{pgfparserx status}
      {%
        \pgfparserx@msg
          {| \space|= (all) unknown for '\meaning\pgfparserxtoken' executed}%
      }
      {}%
  }

% macros for string comparison
\begingroup
\lccode`\P=`\P   \lccode`\G=`\G   \lccode`\F=`\F
\lccode`\:=`\X   \lccode`\;=`\r   \lccode`\A=`\-
\lowercase
  {%
    \endgroup
    \def\pgfparserx@mark{APGFparse;:mark-}%
    \def\pgfparserx@stop{APGFparse;:stop-}%
  }%
\def\pgfparserx@final{final}
\def\pgfparserx@noarg{noarg}
\def\pgfparserx@blankspace{blank space}

% counters
\newcount\pgfparserxdef@arg@count

% shortcuts for special names
\def\pgfparserx@name#1%
  {%
    pgfparserx #1%
  }
\def\pgfparserx@rule#1%
  {%
    \pgfparserx@current\space #1\space \meaning\pgfparserxtoken
  }
\def\pgfparserx@type#1%
  {%
    \pgfparserx@rule{#1} type%
  }
\def\pgfparserx@code#1%
  {%
    \pgfparserx@rule{#1} code%
  }
\def\pgfparserx@undefined#1%
  {%
    \pgfparserx@current\space #1\space unknown%
  }
\def\pgfparserx@ifname#1%
  {%
    pgfparserxconditional #1%
  }

% conditionals
\def\pgfparserx@if#1%
  {%
    \pgfutil@ifundefined{if\pgfparserx@ifname{#1}}
      {%
        \pgferror{Undefined conditional '#1'. Falling back to false}%
        \iffalse
      }
      {\csname if\pgfparserx@ifname{#1}\endcsname}%
      \pgfparserx@fi@BTb
    \fi
    \pgfutil@secondoftwo
  }
\def\pgfparserx@newif#1%
  {%
    \csname newif\expandafter\endcsname
    \csname if\pgfparserx@ifname{#1}\endcsname
  }
\pgfparserx@newif{every parser silent}
\pgfparserx@newif{pgfparserxdef abort}
\pgfparserx@newif{pgfparserx status}

% pgfparserxdef
\pgfutil@protected\def\pgfparserxdef#1#2%
  {%
    \def\pgfparserxdef@twoargs{{#1}{#2}}%
    \pgfparserxdef@a
  }
\pgfutil@protected\def\pgfparserxdef@a
  {%
    \futurelet\pgfparserxdef@arg\pgfparserxdef@b
  }
\def\pgfparserxdef@b
  {%
    \ifx\pgfparserxdef@arg\pgfutil@sptoken
      % will also gobble the \pgfparserxdef@c after \fi
      \pgfparserxdef@gobble@space
    \fi
    \pgfparserxdef@c
  }
\pgfutil@protected\def\pgfparserxdef@gobble@space\fi#1%
  {%
    \fi
    \afterassignment\pgfparserxdef@a
    \let\pgfparserx@ignored@token= % space after = to get spaces, too
  }
\pgfutil@protected\def\pgfparserxdef@c#1%
  {%
    \ifx\pgfparserxdef@arg\bgroup
      \pgfparserx@fi@BTb
    \fi
    \pgfutil@secondoftwo
    {%
      \def\pgfparserxdef@arg{#1}%
      \ifx\pgfparserx@blankspace\pgfparserxdef@arg
        \pgfparserx@fi@BTb
      \fi
      \pgfutil@secondoftwo
      {\expandafter\pgfparserxdef@d\pgfparserxdef@twoargs{blank space \space}}
      {\expandafter\pgfparserxdef@d\pgfparserxdef@twoargs{#1}}%
    }
    {%
      \expandafter\pgfparserxdef@d\pgfparserxdef@twoargs
        {\meaning\pgfparserxdef@arg}%
    }%
  }
\long\def\pgfparserxdef@d#1#2#3%
  {%
    \pgfparserx@oarg@Oarg{}{\pgfparserxdef@e{#1}{#2}{#3}}%
  }
\pgfutil@protected\long\def\pgfparserxdef@e#1#2#3#4#5%
  {%
    \pgfparserx@initiate@parser{#1}%
    \csname\pgfparserx@ifname{pgfparserxdef abort}false\endcsname
    % check how many arguments are used, also parses whether the argument string
    % is valid
    \pgfparserxdef@arg@count=0
    \pgfparserxdef@argcount#4\pgfparserx@mark
    \ifnum\pgfparserxdef@arg@count>9
      \pgferror
        {%
          Too many arguments for parser rule. A maximum of 9 parameters is
          supported.%
        }%
      \csname\pgfparserx@ifname{pgfparserxdef abort}true\endcsname
    \fi
    % define what the code does
    \pgfparserx@if{pgfparserxdef abort}{}
      {%
        % use a macro as flag that there is a rule for the specified state and
        % hold information how to parse it
        \ifnum\pgfparserxdef@arg@count=0
          \pgfutil@namedef{\pgfparserx@name{#1} #2 #3 type}{noarg}%
          \pgfutil@namedef{\pgfparserx@name{#1} #2 #3 code}%
            {#5\pgfparserx@getnexttoken}%
        \else
          \pgfutil@namedef{\pgfparserx@name{#1} #2 #3 type}{#4}%
          \expandafter\pgfparserxdef@argstring@def\the\pgfparserxdef@arg@count
          \pgfutil@namedef{\pgfparserx@name{#1} #2 #3 code\expandafter}%
            \pgfparserxdef@argstring##1##2##3##4##5##6##7##8##9%
            {#5\pgfparserx@getnexttoken}%
        \fi
      }%
  }
\def\pgfparserxdef@argstring@def#1%
  {%
    \def\pgfparserxdef@argstring##1#1##2##{##1#1}%
  }
\def\pgfparserxdef@argcount#1%
  {%
    \ifx\pgfparserx@mark#1%
      \pgfparserx@fi@Bg
    \fi
    \pgfutil@firstofone
    {%
      \pgfutil@IfUndefined{pgfparserxdef@argcount@#1}
        {%
          \pgferror{Unknown argument type '#1'. Aborting rule definition}%
          \pgfparserxdef@argcount@abort
        }
        {\csname pgfparserxdef@argcount@#1\endcsname}%
    }%
  }
\pgfutil@protected\long\def\pgfparserxdef@argcount@abort#1\pgfparserx@mark
  {%
    \csname\pgfparserx@ifname{pgfparserxdef abort}true\endcsname
  }
\def\pgfparserxdef@argcount@m
  {%
    \advance\pgfparserxdef@arg@count1
    \pgfparserxdef@argcount
  }
\let\pgfparserxdef@argcount@o\pgfparserxdef@argcount@m
\def\pgfparserxdef@argcount@O#1%
  {%
    \advance\pgfparserxdef@arg@count1
    \pgfparserxdef@argcount
  }
\let\pgfparserxdef@argcount@r\pgfparserxdef@argcount@O
\let\pgfparserxdef@argcount@t\pgfparserxdef@argcount@O
\def\pgfparserxdef@argcount@d#1#2%
  {%
    \advance\pgfparserxdef@arg@count1
    \pgfparserxdef@argcount
  }
\def\pgfparserxdef@argcount@D#1#2#3%
  {%
    \advance\pgfparserxdef@arg@count1
    \pgfparserxdef@argcount
  }

\pgfutil@protected\def\pgfparserx@initiate@parser#1%
  {%
    % if that is the first rule for the parser
    \pgfutil@IfUndefined{if\pgfparserx@ifname{#1 silent}}%
      {%
        % define space to be a no-op
        \pgfutil@namedef{\pgfparserx@name{#1} all blank space \space\space type}
          {noarg}%
        \pgfutil@namedef{\pgfparserx@name{#1} all blank space \space\space code}
          {\pgfparserx@getnexttoken}%
        % define a new conditional whether the parser silently ignores unknown
        % tokens
        \pgfparserx@newif{#1 silent}%
        \pgfparserxset
          {%
            #1/silent/.is if=\pgfparserx@ifname{#1 silent}%
          }%
      }
      {}%
  }

% pgfparserxlet
\pgfutil@protected\def\pgfparserxlet#1#2%
  {%
    \def\pgfparserxlet@parser@a{#1}%
    \def\pgfparserxlet@state@a{#2}%
    \pgfparserxlet@a
  }
\pgfutil@protected\def\pgfparserxlet@a
  {%
    \futurelet\pgfparserxlet@arga\pgfparserxlet@b
  }
\def\pgfparserxlet@b
  {%
    \ifx\pgfparserxlet@arga\pgfutil@sptoken
      % will also gobble the \pgfparserxlet@c after \fi
      \pgfparserxlet@gobble@space\pgfparserxlet@a
    \fi
    \pgfparserxlet@c
  }
\pgfutil@protected\def\pgfparserxlet@gobble@space#1\fi#2%
  {%
    \fi
    \afterassignment#1%
    \let\pgfparserx@ignored@token= % space after = to get spaces, too
  }
\pgfutil@protected\def\pgfparserxlet@c#1%
  {%
    \ifx\pgfparserxlet@arga\bgroup
      \pgfparserx@fi@BTb
    \fi
    \pgfutil@secondoftwo
    {%
      \def\pgfparserxlet@arga{#1}%
      \ifx\pgfparserx@blankspace\pgfparserxlet@arga
        \pgfparserx@fi@Bb
      \fi
      \pgfutil@gobble
      {\def\pgfparserxlet@arga{blank space \space}}%
    }
    {\edef\pgfparserxlet@arga{\meaning\pgfparserxlet@arga}}%
    \pgfparserx@oarg@oarg\pgfparserxlet@d
  }
\pgfutil@protected\def\pgfparserxlet@d#1%
  {%
    \pgfparserxifmark{#1}
      {%
        \let\pgfparserxlet@parser@b\pgfparserxlet@parser@a
        \let\pgfparserxlet@state@b\pgfparserxlet@state@a
        \pgfparserxlet@f
      }
      {\pgfparserx@oarg@oarg{\pgfparserxlet@e{#1}}}%
  }
\pgfutil@protected\def\pgfparserxlet@e#1#2%
  {%
    \pgfparserxifmark{#2}
      {%
        \let\pgfparserxlet@parser@b\pgfparserxlet@parser@a
        \def\pgfparserxlet@state@b{#1}%
      }
      {%
        \def\pgfparserxlet@parser@b{#1}%
        \def\pgfparserxlet@state@b{#2}%
      }
    \pgfparserxlet@f
  }
\pgfutil@protected\def\pgfparserxlet@f
  {%
    \futurelet\pgfparserxlet@argb\pgfparserxlet@g
  }
\def\pgfparserxlet@g
  {%
    \ifx\pgfparserxlet@argb\pgfutil@sptoken
      % will also gobble the \pgfparserxlet@c after \fi
      \pgfparserxlet@gobble@space\pgfparserxlet@f
    \fi
    \pgfparserxlet@h
  }
\pgfutil@protected\def\pgfparserxlet@h#1%
  {%
    \ifx\pgfparserxlet@argb\bgroup
      \pgfparserx@fi@BTb
    \fi
    \pgfutil@secondoftwo
    {%
      \def\pgfparserxlet@argb{#1}%
      \ifx\pgfparserx@blankspace\pgfparserxlet@argb
        \pgfparserx@fi@Bb
      \fi
      {\def\pgfparserxlet@argb{blank space \space}}%
    }
    {\edef\pgfparserxlet@argb{\meaning\pgfparserxlet@argb}}%
    \pgfparserxlet@i
      \pgfparserxlet@parser@a
      \pgfparserxlet@state@a
      \pgfparserxlet@arga
      \pgfparserxlet@parser@b
      \pgfparserxlet@state@b
      \pgfparserxlet@argb
  }
\def\pgfparserxlet@i#1#2#3#4#5#6%
  {%
    \pgfutil@IfUndefined{\pgfparserx@name{#4} #5 #6 type}
      {%
        \pgferror
          {%
            There is no rule defined for parser '#4' in state '#5' for '#6'.
            Aborting \string\pgfparserxlet
          }%
      }
      {%
        \pgfparserx@initiate@parser{#1}%
        \pgfparserxlet@let{#1}{#2}{#3}{#4}{#5}{#6}{type}%
        \pgfparserxlet@let{#1}{#2}{#3}{#4}{#5}{#6}{code}%
      }%
  }
\def\pgfparserxlet@let#1#2#3#4#5#6#7%
  {%
    \expandafter\let\csname\pgfparserx@name{#1} #2 #3 #7\expandafter\endcsname
                    \csname\pgfparserx@name{#4} #5 #6 #7\endcsname
  }

\pgfutil@protected\long\def\pgfparserxdefunknown#1#2#3%
  {%
    \pgfparserx@initiate@parser{#1}%
    \pgfutil@namedef{\pgfparserx@name{#1} #2 unknown}{#3}%
  }
\pgfutil@protected\long\def\pgfparserxdeffinal#1#2%
  {%
    \pgfutil@namedef{\pgfparserx@name{#1} final action}{#2}%
  }
\pgfutil@protected\def\pgfparserxswitch#1%
  {%
    \pgfparserx@status@state{#1}%
    \def\pgfparserxstate{#1}%
  }
\pgfutil@protected\def\pgfparserxrun#1%
  {%
    \pgfutil@IfUndefined{if\pgfparserx@ifname{#1 silent}}
      {\pgferror{No parser named '#1' defined}}
      {%
        \edef\pgfparserx@current{\pgfparserx@name{#1}}%
        \def\pgfparserx@usersname{#1}%
        \pgfparserx@status@start{#1}%
        \pgfparserxswitch{initial}%
        \pgfparserx@getnexttoken
      }%
  }
% get next token
\pgfutil@protected\def\pgfparserx@getnexttoken
  {%
    \ifx\pgfparserxstate\pgfparserx@final
      \pgfparserx@fi@BTb
    \fi
    \pgfutil@secondoftwo
    {%
      \pgfutil@IfUndefined{\pgfparserx@current\space final action}
        {}
        {\csname \pgfparserx@current\space final action\endcsname}%
    }
    {%
      \futurelet\pgfparserxtoken\pgfparserx@getnexttoken@a
    }%
  }
\pgfutil@protected\def\pgfparserx@getnexttoken@a
  {%
    \pgfparserx@getnexttoken@iftokengobble\bgroup\bgroup
      {%
        \pgfparserx@getnexttoken@iftokengobble\egroup\egroup
          {%
            \pgfparserx@getnexttoken@iftokengobble\pgfutil@sptoken{ }%
              {\pgfparserx@getnexttoken@defchar}%
          }%
      }%
  }
\pgfutil@protected\def\pgfparserx@getnexttoken@iftokengobble#1#2%
  {%
    \ifx\pgfparserxtoken#1%
      \def\pgfparserxletter{#2}%
      \pgfparserx@getnexttoken@gobble
    \fi
    \pgfutil@firstofone
  }
\pgfutil@protected\def\pgfparserx@getnexttoken@gobble\fi#1#2%
  {%
    \fi
    \afterassignment\pgfparserx@getnexttoken@b
    \let\pgfparserx@ignored@token= % gobble should gobble a space
  }
\pgfutil@protected\long\def\pgfparserx@getnexttoken@defchar#1%
  {%
    \def\pgfparserxletter{#1}%
    \pgfparserx@getnexttoken@b
  }
\pgfutil@protected\def\pgfparserx@getnexttoken@b
  {%
    \pgfutil@IfUndefined{\pgfparserx@type{\pgfparserxstate}}
      {%
        \pgfutil@IfUndefined{\pgfparserx@type{all}}
          {%
            \pgfparserx@if{every parser silent}{}
              {%
                \pgfparserx@if{\pgfparserx@usersname\space silent}{}
                  {%
                    \pgferror
                      {%
                        No rule for parser '\pgfparserx@usersname' in state
                        '\pgfparserxstate' for '\meaning\pgfparserxtoken'.
                        Ignoring token%
                      }%
                  }%
              }%
            \pgfutil@IfUndefined{\pgfparserx@undefined{\pgfparserxstate}}
              {%
                \pgfutil@IfUndefined{\pgfparserx@undefined{all}}
                  {}
                  {%
                    \pgfparserx@status@rule@undefined@all
                    \csname\pgfparserx@undefined{all}\endcsname
                  }%
              }
              {%
                \pgfparserx@status@rule@undefined
                \csname\pgfparserx@undefined{\pgfparserxstate}\endcsname
              }%
            \pgfparserx@getnexttoken
          }
          {%
            \pgfparserx@status@rule@all
            \pgfparserx@handle{all}%
          }%
      }
      {%
        \pgfparserx@status@rule
        \pgfparserx@handle{\pgfparserxstate}%
      }%
  }

% handle arguments
\pgfutil@protected\def\pgfparserx@handle#1%
  {%
    \expandafter\ifx\csname\pgfparserx@type{#1}\endcsname\pgfparserx@noarg
      \pgfparserx@fi@BTb
    \fi
    \pgfutil@secondoftwo
    {\csname\pgfparserx@code{#1}\endcsname}
    {\pgfparserx@handle@args{#1}}%
  }
\pgfutil@protected\def\pgfparserx@handle@args#1%
  {%
    \def\pgfparserx@stateorall{#1}%
    \expandafter\expandafter\expandafter\pgfparserx@handle@args@a
    \csname\pgfparserx@type{#1}\endcsname\pgfparserx@mark\pgfparserx@stop
  }
\def\pgfparserx@handle@args@a
  {%
    \pgfparserx@handle@args@b{}%
  }
\pgfutil@protected\long\def\pgfparserx@handle@args@b#1#2#3\pgfparserx@stop
  {%
    \pgfparserxifmark{#2}
      {\csname\pgfparserx@code{\pgfparserx@stateorall}\endcsname#1}
      {%
        \pgfutil@IfUndefined{pgfparserx@handle@args@#2}
          {\pgferror{Unknown argument type '#2'}}
          {\csname pgfparserx@handle@args@#2\endcsname{#1}{#3}}%
      }%
  }
\long\def\pgfparserx@handle@args@c#1#2%
  {%
    \pgfparserxifmark{#2}
      {\csname\pgfparserx@code{\pgfparserx@stateorall}\endcsname#1}
      {\pgfparserx@handle@args@b{#1}#2\pgfparserx@stop}%
  }
\long\def\pgfparserx@handle@args@m#1#2#3%
  {%
    \pgfparserx@handle@args@c{#1{#3}}{#2}%
  }
\pgfutil@protected\long\def\pgfparserx@handle@args@o#1#2%
  {%
    \pgfparserx@oarg@oarg{\pgfparserx@handle@args@m{#1}{#2}}%
  }
\long\def\pgfparserx@handle@args@O#1#2%
  {%
    \pgfparserx@handle@args@O@{#1}#2\pgfparserx@stop
  }
\pgfutil@protected\long\def\pgfparserx@handle@args@O@#1#2#3\pgfparserx@stop
  {%
    \pgfparserx@oarg@Oarg{#2}{\pgfparserx@handle@args@m{#1}{#3}}%
  }
\long\def\pgfparserx@handle@args@d#1#2%
  {%
    \pgfparserx@handle@args@d@{#1}#2\pgfparserx@stop
  }
\pgfutil@protected\long\def\pgfparserx@handle@args@d@#1#2#3#4\pgfparserx@stop
  {%
    \pgfparserx@oarg@darg{#2}{#3}{\pgfparserx@handle@args@m{#1}{#4}}%
  }
\long\def\pgfparserx@handle@args@D#1#2%
  {%
    \pgfparserx@handle@args@D@{#1}#2\pgfparserx@stop
  }
\pgfutil@protected\long\def\pgfparserx@handle@args@D@#1#2#3#4#5\pgfparserx@stop
  {%
    \pgfparserx@oarg@Darg{#2}{#3}{#4}{\pgfparserx@handle@args@m{#1}{#5}}%
  }
\long\def\pgfparserx@handle@args@r#1#2%
  {%
    \pgfparserx@handle@args@r@a{#1}#2\pgfparserx@stop
  }
\pgfutil@protected\long\def\pgfparserx@handle@args@r@a#1#2#3\pgfparserx@stop
  {%
    \def\pgfparserx@handle@args@r@b##1#2%
      {\pgfparserx@handle@args@c{#1{##1}}{#3}}%
    \pgfparserx@handle@args@r@b
  }
\long\def\pgfparserx@handle@args@t#1#2%
  {%
    \pgfparserx@handle@args@t@{#1}#2\pgfparserx@stop
  }
\pgfutil@protected\long\def\pgfparserx@handle@args@t@#1#2#3\pgfparserx@stop
  {%
    \pgfparserx@oarg@targ{#2}{\pgfparserx@handle@args@m{#1}{#3}}%
  }

\pgfutil@protected\long\def\pgfparserxreinsert#1%
  {%
    \ifx\pgfparserx@getnexttoken#1%
      \pgfparserx@fi@BTb
    \fi
    \@secondoftwo
    {\expandafter\pgfparserx@getnexttoken\pgfparserxletter}
    {%
      \pgferror
        {%
          Misuse of \string\pgfparserxreinsert\space detected. You should only
          use it at the the end of an action code%
        }%
      #1%
    }%
  }

\endinput
