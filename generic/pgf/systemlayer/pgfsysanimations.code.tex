...
\def\pgfsyssnapshot#1#2#3{%
  ...
  \def\pgfsysanim@temp{#2}%
  \def\pgfsysanim@snapevents{#3}%
  \ifx\pgfsysanim@temp\pgfsysanim@plus@text%
...
\def\pgfsysanim@do@snap#1{%
  {%
    ...
    \else%
      ...
      \else%
        % Attach to object:
        \expandafter\expandafter\expandafter\pgfsysanim@call@attacher\expandafter\expandafter\expandafter{\expandafter\pgfsysanim@code@begin\expandafter}\expandafter{\pgfsysanim@code@end}{}%
      \fi%
    \fi%
  }%
}
...
\def\pgfsysanimkeyoffset#1#2{%
  ...
  \ifpgfsysanim@is@snap%
    \def\pgfsys@temp{#2}%
    \ifx\pgfsys@temp\pgf@sys@begin@text%
      \pgferror{begin offset not yet implemented}%
    \fi%
  \fi%
}
...
% Description:
%
% Calls \pgfsys@animation@sync(begin|end) Furthermore, if #4 is
% "begin" (for both syncbegin and syncend), the specified sync is
% recorded so that it can later be analysed for the computation needed
% for a snapshot. 
...
\def\pgfsysanimkeysyncbegin#1#2#3#4{%
  ...
  \ifpgfsysanim@is@snap%
    \def\pgfsys@temp{#4}%
    \ifx\pgfsys@temp\pgf@sys@begin@text%
      \pgferror{sync begin not yet implemented}%
    \fi%
  \fi%
}
...
\def\pgfsysanimkeysyncend#1#2#3#4{%
  ...
  \ifpgfsysanim@is@snap%
    \def\pgfsys@temp{#4}%
    \ifx\pgfsys@temp\pgf@sys@begin@text%
      \pgferror{sync end not yet implemented}%
    \fi%
  \fi%
}
...
% Description:
%
% Calls \pgfsys@animation@event and, if #5 is "begin", the specified
% event is recorded so that it can later be analysed for the
% computation needed for a snapshot. 
...
\def\pgfsysanimkeyevent#1#2#3#4#5{%
  ...
  \ifpgfsysanim@is@snap%
    \def\pgfsys@temp{#5}%
    \ifx\pgfsys@temp\pgf@sys@begin@text%
      \pgferror{event not yet implemented}%
    \fi%
  \fi%
}
...
% Description:
%
% Calls \pgfsys@animation@repeat@event and, if #5 is "begin", the specified
% event is recorded so that it can later be analysed for the
% computation needed for a snapshot. 
...
\def\pgfsysanimkeyrepeatevent#1#2#3#4#5{%
  ...
  \ifpgfsysanim@is@snap%
    \def\pgfsys@temp{#5}%
    \ifx\pgfsys@temp\pgf@sys@begin@text%
      \pgferror{repeat event not yet implemented}%
    \fi%
  \fi%
}
...
% Description:
%
% Calls \pgfsys@animation@accesskey and, if #3 is "begin", the
% specified event is recorded so that it can later be analysed for the 
% computation needed for a snapshot. 
...
\def\pgfsysanimkeyaccesskey#1#2#3{%
  ...
  \ifpgfsysanim@is@snap%
    \def\pgfsys@temp{#3}%
    \ifx\pgfsys@temp\pgf@sys@begin@text%
      \pgferror{access key not yet implemented}%
    \fi%
  \fi%
}
...
\def\pgfsysanim@compute@interval{%
  ...
  \else%
    ...
    \else%
      \pgferror{repeats not yet implement for snapshots}%
    \fi%
    \ifpgfsysanim@freezeatend%
      \ifx\pgfsysanim@next@time\relax%
    ...
    \else%
      ...
      \else%
        ...
        \else
          \ifdim\pgfsysanim@last@time pt=\pgfsysanim@snaptime pt%
            \let\pgfsysanim@prev@val\relax%
            \let\pgfsysanim@prev@time\relax%
            \let\pgfsysanim@prev@spline\relax%
            \let\pgfsysanim@next@val\relax%
...
% "Adjust" the snap time: In case one or more events are met, the snap time is
% shifted forward by the first event that is met. May also set the
% snap time "" in case event is met.
\def\pgfsysanim@adjust@time{%
  % Not yet implemented: Must iterate over all trigged events and
  % cross-reference them with the snapshot's events.
}



\def\pgfsysanim@test@time#1#2#3#4#5#6{%
  \ifx\pgfsysanim@first@time\relax%
  ...
  % #6 = value
  \ifdim#1pt<\pgfsysanim@snaptime pt%
...
\def\pgfsysanim@compute@fractions{%
  ...
  \else%
    ...
    \else%
      ...
      \pgfmathsubtract@{\pgfsysanim@snaptime}{\pgfsysanim@prev@time}%
