...
\def\pgf@decorate@@invoke#1#2#3#4#5\pgf@stop{%
	...
	{%
	  ...
	  {%
	    ...
	    \let\externalcode\pgf@decorate@stateexternalcode%
...
% Execute code outside a state TeX-group.
%
%
\def\pgf@decorate@stateexternalcode#1{%
	\egroup%
	#1%
	\bgroup%
		\pgftransformreset%
...
\pgfkeys{
  ...
  /pgf/decoration automaton/switch if subpath less than/.code=\pgf@decorate@switch@ifsubpath#1\pgf@stop,%
...
\pgfdeclaredecoration{lineto}{initial}{
  \state{initial}[switch if subpath less than=\pgfdecoratedremainingdistance to subpath,
                  width=\pgfdecoratedremainingdistance]
  {
  }
  \state{subpath}[width=\pgfdecoratedsubpathlength,next state=initial]
  {
    \pgfpathlineto{\pgfpointdecoratedsubpathlast}
  }
...
\pgfset{%
  /pgf/segment amplitude/.style={/pgf/decoration={amplitude=#1}},
  ...
  /pgf/segment object length/.style={/pgf/decoration={shape width=#1}}}
