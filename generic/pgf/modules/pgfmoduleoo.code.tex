...
\long\def\pgfooclass@(#1)#2#3{%
  ...
  \expandafter\ifx\csname pgfooY\pgfoo@classname.@pgfooinit\endcsname\relax\else
    \PackageError{pgfoo}{class #2 is already defined}{}%
  \fi
  \pgfoo@ciii{#2}{#1}%
...
%    \PackageError{pgfoo}{An attempt to derive from the same class twice}{}%
%  \else\pgfoo@escapeif{%
    \pgfutil@in@{,#3,}{,#7}%
...
\def\pgfoo@ciii@merge@checkHead@P(#1)(#2)(#3){%
  \if\relax#3\relax % no more good head candidates!
    \PackageError{pgfoo}{Bad MRO}{There is no good (monotonic etc.)
...
\def\pgfoo@caller#1.#2(#3){%
  ...
  \ifx\pgfoo@caller@temp\relax
    ...
    \ifx\pgfoo@caller@temp\relax
      \PackageError{pgfoo}{Object #1 has no method #2}{}
    \fi
  \fi    
  \def\pgf@marshal{%
