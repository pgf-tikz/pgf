...
\def\pgfmath@parsescientific@{%
	...
	\edef\pgfmath@parsesci@exp{\the\c@pgfmath@parsecounta}%
	\let\pgfmath@next\pgfmath@@parsescientific%
	%
	\ifnum\c@pgfmath@parsecounta<0\relax%
		\c@pgfmath@parsecounta-\c@pgfmath@parsecounta%
	\fi%
	\ifnum\c@pgfmath@parsecounta>4\relax%
		\let\pgfmath@next\pgfmath@parsescientific@toobig%
	\else%
		\ifnum\c@pgfmath@parsecounta>3\relax%
			\pgfmath@x\pgfmath@parsesci@temp pt\relax%
			\ifdim\pgfmath@x<0pt\relax%
				\pgfmath@x-\pgfmath@x%
			\fi%
			\ifdim\pgfmath@x>1.638399999pt\relax%
				\let\pgfmath@next\pgfmath@parsescientific@toobig%
			\fi%
		\fi%			
	\fi%
...
\def\pgfmath@echar{e}
\def\pgfmath@parsescientific@toobig#1\pgfmath@empty{%
	%
	% Exit parser immediately...
	%
	\edef\pgfmathresult{\pgfmath@parsesci@temp\pgfmath@echar\pgfmath@parsesci@exp}%
	\pgfmath@smuggleone\pgfmathresult%
	\endgroup%
	\ifx\pgfmathfloat\undefined%
		\pgfmath@error{I cannot process `\pgfmathresult'. It is too big}{}%
	\else%
		\expandafter\pgfmathfloat\expandafter{\pgfmathresult}%		
	\fi%
}

\def\pgfmath@@parsescientific{%
	\expandafter\pgfmathscientific\expandafter{\pgfmath@parsesci@temp}%
		{\pgfmath@parsesci@exp}%
