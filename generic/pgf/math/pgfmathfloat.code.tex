...
\def\pgfmathfloatparsenumber#1{%
	...
	\expandafter\pgfflt@impl\pgfmathresult\pgfmathfloat@EOI
...
% #1 maybe a macro (it will be expanded ONCE)
...
\def\pgfmathlog@float#1{%
	\begingroup%
		...
		\ifnum\pgfmathfloat@a@S=1
			...
			\expandafter\pgfmathln@\expandafter{\the\pgfmathfloat@a@Mtok}%
...
\def\pgfmathfloat@EOI@{pgfflt@EOI}%
...
\def\pgfflt@error#1\pgfmathfloat@EOI{%
	\begingroup
	\pgfmathfloat@a@Mtok={#1}%
...
\def\pgfflt@finish#1\pgfmathfloat@EOI{}
...
\expandafter\def\csname pgffltB@\pgfmathfloat@EOI\endcsname{%
	\pgfmathfloat@a@S=0\relax%
	\pgfmathfloat@a@Mtok={0.0}%
	\pgfmathfloat@a@E=0}%
...
\expandafter\def\csname pgffltC@\pgfmathfloat@EOI\endcsname{%
	\pgfmathfloat@a@S=0\relax%
	\pgfmathfloat@a@Mtok={0.0}%
	\pgfmathfloat@a@E=0}%
...
\expandafter\def\csname pgffltD@\pgfmathfloat@EOI\endcsname{}% NOP
...
\def\pgfflt@readexponent#1\pgfmathfloat@EOI{%
...
\expandafter\def\csname pgffltE@\pgfmathfloat@EOI\endcsname{}% NOP
...
\expandafter\def\csname pgffltF@\pgfmathfloat@EOI\endcsname{}% NOP
...
\expandafter\def\csname pgffltG@\pgfmathfloat@EOI\endcsname{}% NOP
