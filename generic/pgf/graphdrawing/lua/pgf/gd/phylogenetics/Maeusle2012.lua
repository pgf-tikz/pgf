local Maeusle2012 = {}
Maeusle2012._index = Maeusle2012

require("pgf.gd.trees").Maeusle2012 = Maeusle2012
...
function Maeusle2012:run(phy_tree)
  -- store the phylogenetic tree object 
  self.phy_tree = phy_tree

  local g = self.phy_tree.digraph
  local layout = g.storage.layout  
  -- determine the root
  if not g.storage.root then
		g.storage.root = self:getRoot()
	end
  
  -- set the x- and y-positions according to the user's layout specifications
  if (layout == 'rectangular phylogram' or layout == 'rooted rectangular phylogram') then
    self:setPosForRectangularLayout(g.storage.root)
  elseif (layout == 'rooted straight phylogram' or layout == 'straight phylogram') then
		self:setXPos(g.storage.root)
    self:setYPosForStraightLayout(g.storage.root)
  
  -- unrooted layouts:
  elseif (layout == 'unrooted rectangular phylogram' or layout == 'unrooted straight phylogram') then
    -- get the roots of the two subtrees to be used as the "center" of
    -- the tree:
    local v1, v2
		if not g.storage.neighbor_of_root then 
			local center_vertices = self:computeCenterOfPath() 
    	v1 = center_vertices.root
    	v2 = center_vertices.neighbor_of_root
		else -- if the center of the path has already been computed, use this info!
			v1 = g.storage.root
			v2 = g.storage.neighbor_of_root
		end
    
    -- set the x- and y-positions
    if (layout == 'unrooted rectangular phylogram') then
      self:setPosForUnrootedRectangular(v2,v1)
    elseif (layout == 'unrooted straight phylogram') then
	    g.storage.straight_edges = true
      self:setPosForUnrootedStraight(v2,v1)
    end
  end
end 
...
function Maeusle2012:createFinalEdges()
  local g = self.phy_tree.digraph
...
function Maeusle2012:edgeLength(vertex1, vertex2)
  local length
  if vertex1.storage.length then
    length = vertex1.storage.length[vertex2]
  else
    length = self:distance(vertex1, vertex2)
  end
  return length
end
...
function Maeusle2012:setPosForRectangularLayout(vertex, values, vertex2)
  ...
  if (not values.visited[vertex]) then values.visited[vertex] = vertex end
  ...
  if vertex_is_leaf then
    ...
    -- edge length
    vertex.pos.y = - vertex.storage[self.phy_tree].layer_pre
    ...
    if #values.leaves > 1 then
      ...
      local ideal_distance = layered.ideal_sibling_distance(self.phy_tree, self.phy_tree.digraph, vertex, left_sibling )
  ...
  vertex.pos.y = vertex.pos.y + (values.length * self.phy_tree.digraph.storage.factor)
...
function Maeusle2012:setYPosForStraightLayout(vertex, values, vertex2)
  ...
  for _, arc in ipairs (self.phy_tree.digraph:outgoing(vertex)) do
    if not values.visited[arc.head] and arc.head ~= vertex2 then
      ...
      -- theorem
			local a
      local l = self:edgeLength(vertex, arc.head) * self.phy_tree.digraph.storage.factor
...
function Maeusle2012:correctXPos(vertex1, vertex2)
  ...
  local function x_correction(vertex, values, vertex2)
    ...
    for _, arc in ipairs (Maeusle2012.phy_tree.digraph:outgoing(vertex)) do
      if not values.visited[arc.head] and arc.head ~= vertex2 then
    ...
    if not self.phy_tree.digraph.storage.straight_edges then
  		for i,child in ipairs(children) do
    		Maeusle2012:bendEdge90Degree(child, vertex)
			end
		end
...
function Maeusle2012:setPosForUnrootedRectangular(vertex1, vertex2)
  ...
  self:correctXPos(vertex1, vertex2)
end
...
function Maeusle2012:setPosForUnrootedStraight(vertex1, vertex2)
  ...
  local vals={
    length = self:edgeLength(vertex1, vertex2) * self.phy_tree.digraph.storage.factor,
    visited={},
    leaves={},
    path={},
    second_subtree = true
...
function Maeusle2012:bendEdge90Degree(head, tail)
  local arc = self.phy_tree.digraph:arc(head, tail)
...
function Maeusle2012:findLongestPath()
  local starting_point = self.phy_tree.digraph.vertices[1] -- begin at any vertex
 ...
 -- looks for the longest path and identifies its end-point
  local function find_head_of_longest_path(path_lengths, paths)
    ...
    local node
		...
		for _, vertex in ipairs(self.phy_tree.digraph.vertices) do
...
function Maeusle2012:getPathLengthsToLeaves(vertex, values)
  ...
  for _, arc in ipairs (self.phy_tree.digraph:outgoing(vertex)) do
    if not values.visited[arc.head] then
      ...
      vertex_is_leaf = false
      local arc_length
      if self.phy_tree.digraph.storage.bme then -- use lengths stored in here by the BME algorithm
        arc_length = vertex.storage.length[arc.head]
      else
        arc_length = self:distance(vertex, arc.head)
      end
      values.length = values.length + arc_length
...
function Maeusle2012:getRoot()
  local root, neighbor_of_root, root_info

  -- check whether a root exists (vertex with degree 2)
  local vertices = self:checkForVerticesWithDegree(2)
  
  if (#vertices > 1) then 
    --tree has more than one vertex with degree 2, take first one as
    --root
    root = vertices[1]
  elseif (#vertices == 1) then
    root = vertices[1]
  else
    root_info = self:computeCenterOfPath()
		root = root_info.root
		self.phy_tree.digraph.storage.neighbor_of_root = root_info.neighbor_of_root
  end
...
--- Checks whether the tree has nodes with a specified degree
--  
--  @param degree The degree of the nodes we're looking for 
--  @return A table of vertices with that degree
function Maeusle2012:checkForVerticesWithDegree(degree)
  local arcs = self.phy_tree.digraph.arcs
  local vertices = {}

  -- Counting the number of times a specific node is the head of an arc
  local head_count = {}
  for _, arc in pairs (arcs) do
    if not head_count[arc.head] then 
      head_count[arc.head] = 0 
    end
    head_count[arc.head] = head_count[arc.head] + 1
  end

  -- Searching the vertices
  for vertex, count in pairs(head_count) do 
...
--- Finds the two vertices closest to the center of a path
--
...
function Maeusle2012:computeCenterOfPath()
  ...
  return { root = root, neighbor_of_root = neighbor_of_root }
end


--- Gets the distance between two nodes as specified in their options
--  or storage fields. This is a function for distance-based
--  algorithms.
...
function Maeusle2012:distance(a, b)
