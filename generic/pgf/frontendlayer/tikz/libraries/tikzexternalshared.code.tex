...
%	system call/.initial={pdflatex -interaction=batchmode -jobname '\image' '\texsource'; pdftops -eps \image.pdf},
	...
	up to date check/md5/.code={%
		\pgfutil@ifundefined{pdfmdfivesum}{%
			% FIXME : this can (will) also happen for luatex!
			% I tried to fix it, but the closest that I came up with
			% is \directlua{tex.print(md5.sumhexa('#1'))}
			% this, however, has a different outcome as \pdfmdfivesum.
			% Since it is always a risk to rely on external lua
			% libraries, I kept it this way:
