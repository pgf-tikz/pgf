...
\def\tikzexternal@externalizefig{%
	% this works, but does not respect catcode changes:
	\tikzexternal@TEXDIALECT@collectpicture\tikzexternal@externalizefig@@
	% FIXME :
	% I need to re-implement the code of
	% \pgf@external@normal [which is invoked if we are not currently in 'grab' mode]
	% and
	% \pgf@external@grab [invoked only in grab mode (--jobname matches)]
	% 
	% -> then, overwrite \tikzpicture and \endtikzpicture without
	%  collecting the contents! Only collect anything if there is a
	%  graphics file!
	%  So:
	%   if grab mode && --jobname matches
	%   	-> grab! (re-implement the code if \pgf@external@grab)
	%  	else
	%  		if graphics exists
	%  			include it and skip image
	%  		else
	%  			process image normally
	%  		fi
	%  	fi
	% This should provide an implementation which doesn't conflict
	% with catcode changes. If I am not mistaken, this is the ONLY
	% place in the external library where catcode changes might hurt.
}
