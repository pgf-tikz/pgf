...
\pgfqkeys{/tikz/external}{
	...
	system call/.initial={pdflatex -interaction=batchmode -jobname '\image' '\texsource'},
%	system call/.initial={pdflatex -interaction=batchmode -jobname '\image' '\texsource'; pdftops -eps \image.pdf},
	...
	verbose IO=true,
	optimize/.is if=tikzexternal@optimize,
	optimize=true,
	only named/.is if=tikzexternal@onlynamed,
	only named/.default=true,
	prefix/.store in=\tikzexternal@filenameprefix,
	prefix=,
}
...
\def\tikzexternalize@opt[#1]#2{%
	...
	\iftikzexternal@optimize
		\ifpgf@external@grabshipout
			...
			\let\includegraphics=\tikzexternal@includegraphics@optimizedaway
		\fi
	\fi
}
...
% These are the ONLY TWO TeX dialect-specific commands which need to be overriden when used with
...
\def\tikzexternal@includegraphics@optimizedaway{%
	\pgfutil@ifnextchar[{%
		\tikzexternal@includegraphics@optimizedaway@
	}{%
		\tikzexternal@includegraphics@optimizedaway@[]%
	}%
}%
\def\tikzexternal@includegraphics@optimizedaway@[#1]#2{%
	\immediate\write16{Image '#2' optimized away because it does not contribute to exported PDF. Use '/tikz/external/optimize=false' to disable this.}%
	[ {\sc Image optimized away because it does not contribute to exported PDF}]}%
	...
	\iftikzexternalexportnext
		...
		\ifx\tikzexternal@nextfile\pgfutil@empty
			...
			\else
				...
				\xdef\pgf@tempa{\the\t@tikzexternal@tmpa\the\t@tikzexternal@tmpb\the\c@tikzexternal@picturenum}%
...
\long\def\tikzexternal@externalizefig@@#1{%
	...
	\iftikzexternal@optimize
		\ifpgf@external@grabshipout
			\let\includegraphics=\tikzexternal@orig@includegraphics
		\fi
	\fi
...
% Throws each tikzpicture away - without further notice.
...
\def\tikzexternal@externalizefig@systemcall@uptodatecheck#1{%
