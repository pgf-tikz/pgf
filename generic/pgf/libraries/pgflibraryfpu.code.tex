...
\def\pgfmathfloat@prepareuninstallcmd#1{%
	% and store backup information:
	\expandafter\let\csname pgfmathfloat@backup@\string#1\endcsname=#1%
	\expandafter\def\expandafter\pgfmathfloat@uninstall\expandafter{\pgfmathfloat@uninstall
...
\def\pgfmathfloat@parser@install{%
	\let\pgfmathfloat@uninstall=\pgfutil@empty
...
\def\pgfmathfloat@parser@install@pgf@two@null@null{%
	...
	\pgfmathfloat@install\pgfmath@parseunits@=\pgfmathfloat@parseunits@pgf@two@null@null
...
\def\pgfmathfloat@parseunits@pgf@two@null@null#1#2{%
	\if#1\pgfmathfloat@POSTFLAGSCHAR% check whether it is a float
...
\def\pgfmathfloat@return@float@pgf@two@null@null#1]{%
	\edef\pgfmathresult{\pgfmath@resulttemp \pgfmathfloat@POSTFLAGSCHAR#1]}%
...
\def\pgfmathfloat@stack@push@operand@single#1\relax{%
	\pgfutil@in@ \pgfmathfloat@POSTFLAGSCHAR{#1}%
	\ifpgfutil@in@
