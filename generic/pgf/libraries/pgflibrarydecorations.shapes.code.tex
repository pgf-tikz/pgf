...
% Keys for decoration: shape backgrounds
%
% shape              : the shape used in the decoration.
% shape sep          : the distance between shape borders.
% shape scaled       : scale the shapes in the decoration along the path.
...
\pgfdeclaredecoration{shape backgrounds}{initialise}
{
      ...
      \else%
        ...
        \ifpgf@lib@shapedecoration@betweenborders%
          ...
          \ifnum\c@pgf@counta>1\relax%
            ...
            \advance\pgf@x\pgfdecorationremainingdistance%
          ...
          \else%
            ...
            \pgf@x\pgfdecorationremainingdistance%
            \ifnum\c@pgf@counta=1\relax%
              \pgf@y.5\pgf@x%
  ...
  \state{before shape}
  [
    width=+\pgf@lib@shapedecoration@beforeshape,
    next state=shape,
    persistent precomputation=
    {
      \ifpgfshapedecorationscaled%
        \ifpgf@lib@shapedecoration@betweenborders%
          \ifx\pgf@lib@shapedecoration@spread\pgfutil@empty%
            ...
            % 
            \pgf@x\pgfdecorationcompleteddistance%
            \advance\pgf@x\pgfdecorationremainingdistance%
            \pgf@xa\pgf@lib@shapedecoration@startwidth\relax%
            \pgf@xa.5\pgf@xa%
            \advance\pgf@xa\pgfdecorationcompleteddistance% c+W/2
            % 
        ...
        \else%
          ...
          \pgf@x\pgfdecorationcompleteddistance%
          \advance\pgf@x\pgfdecorationremainingdistance%
          \pgfmathdivide@{\pgfmath@tonumber{\pgfdecorationcompleteddistance}}{\pgfmath@tonumber{\pgf@x}}%			
        \fi%
        % 
        % Get the new width.
        % 
  ...
  \state{shape}[width=+1sp,next state=after shape]
  {
    ...
    \else%
      \pgftransformrotate{-\pgfdecorationangle}%
    \fi%
    % 
