...
\let\pgfmathparse@orig=\pgfmathparse
...
\def\pgfmathfpparse{%
	\begingroup%
		%
		% Mysteriously, it seems that all \let statements must be 
		% installed here, not within some outer scope (where loads
		% of errors occur).  
		%
		\pgfmathfp@parser@install%
		\afterassignment\pgfmathfpparse@%
		\futurelet\pgfmathfp@token}

\def\pgfmathfpparse@#1{%
		\let\pgfmathpostparse=\relax%
		\ifx\pgfmathfp@token*%
			\pgfmathparse@{\pgfmathfpscale#1}%
		\else%
			\pgfmathparse@{#1}%
		\fi%
...
\def\pgfmathfp@parser@install{%
	...
	\else%
	  %
		...
		%
		\let\pgfmath@parse@startgroup=\pgfmathfp@parse@startgroup%
	\fi%
}%

\def\pgfmathfp@parse@startgroup{%
	\begingroup%
		\let\pgfmath@parse@postgroup=\pgfmath@parse@operator%
		\pgfmath@resetparsingparameters%
		\pgfutil@ifnextchar*{\pgfmath@parse@\pgfmathfpscale}{\pgfmath@parse@}%
}
% Scientific notation.
...
\def\pgfmath@parse@function@e{\pgfmathe@\pgfmath@postfunction}
\def\pgfmathe@{\let\pgfmathresult=\pgfmath@e}
\def\pgfmath@e{2.718281}
