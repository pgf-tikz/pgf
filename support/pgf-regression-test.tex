...
% Typeset arbitrary material into a box and dump the box contents to the log so
% that l3build can compare them against a reference (.tlg) file.  This is the
% workhorse behind the tests generated from the manual's code examples.
\def\BEGINBOXTEST#1{%
  \BEGINTEST{#1}%
  \setbox\PGFTESTBOX=\hbox\bgroup
}
